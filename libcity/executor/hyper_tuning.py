        ...
        if node.name in implicit_stochastic_symbols:
            if node.name not in supported_stochastic_symbols:
                raise ExhaustiveSearchError(
                    "Exhaustive search is only possible \
...
class HyperTuning:
    ...
    def save_result(self, filename=None):
        with open(filename, "w") as fp:
            ...
            fp.write(
                str(
                    self.params2result[self.params2str(self.best_params)][
                        "best_valid_score"
                    ]
                )
                + "\n"
            ...
            fp.write(
                str(
                    self.params2result[self.params2str(self.best_params)]["test_result"]
                )
                + "\n"
            )
            fp.write(
    ...
    def fn(self, params):
        ...
        self._logger.info(str(hyper_config_dict))
        self._logger.info(
            "current best valid score: %.4f" % result_dict["best_valid_score"]
        )
