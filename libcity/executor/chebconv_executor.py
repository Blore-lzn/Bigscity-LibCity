...
class ChebConvExecutor(TrafficStateExecutor):
    ...
    def evaluate(self, test_dataloader):
        ...
        with torch.no_grad():
            ...
            self._logger.info(
                "mae={}, map={}, rmse={}".format(mae.item(), mape.item(), rmse.item())
            )
    ...
    def _train_epoch(self, train_dataloader, epoch_idx, loss_func=None):
        ...
        Returns:
            float: 训练集的损失值
        """
        node_features = torch.FloatTensor(train_dataloader["node_features"]).to(
            self.device
        )
        node_labels = node_features.clone()
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, loss_func=None):
        ...
        Returns:
            float: 验证集的损失值
        """
        node_features = torch.FloatTensor(eval_dataloader["node_features"]).to(
            self.device
        )
        node_labels = node_features.clone()
