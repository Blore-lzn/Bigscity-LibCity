...
class TrafficStateExecutor(AbstractExecutor):
    ...
    def _build_lr_scheduler(self):
        ...
        if self.lr_decay:
            self._logger.info(
                "You select `{}` lr_scheduler.".format(self.lr_scheduler_type.lower())
            )
            if self.lr_scheduler_type.lower() == "multisteplr":
                ...
                self._logger.warning(
                    "Received unrecognized lr_scheduler, "
    ...
    def evaluate(self, test_dataloader):
        ...
        with torch.no_grad():
            ...
            np.savez_compressed(
                os.path.join(self.evaluate_res_dir, filename), **outputs
            )
            self.evaluator.clear()
            self.evaluator.collect(
                {"y_true": torch.tensor(y_truths), "y_pred": torch.tensor(y_preds)}
            )
