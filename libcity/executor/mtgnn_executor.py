...
class MTGNNExecutor(TrafficStateExecutor):
    ...
    def _build_train_loss(self):
        ...
        if self.train_loss.lower() not in [
            ...
            self._logger.info(
                "You select `{}` as train loss function.".format(
                    self.train_loss.lower()
                )
            )
        ...
        def func(batch, idx=None, batches_seen=None):
            ...
            if self.model.training:
                if (
                    batches_seen % self.model.step_size == 0
                    and self.model.task_level < self.model.output_window
                ):
                ...
                if self.model.use_curriculum_learning:
                    return lf(
                        y_predicted[:, : self.model.task_level, :, :],
    ...
    def _train_epoch(
        self, train_dataloader, epoch_idx, batches_seen=None, loss_func=None
    ):
        """
        完成模型一个轮次的训练
    ...
    def _valid_epoch(
        self, eval_dataloader, epoch_idx, batches_seen=None, loss_func=None
    ):
        """
        完成模型一个轮次的评估
