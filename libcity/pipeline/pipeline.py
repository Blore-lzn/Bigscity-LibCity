...
def run_model(
    ...
    # load config
    config = ConfigParser(
        task, model_name, dataset_name, config_file, saved_model, train, other_args
    )
    ...
    logger.info(
        "Begin pipeline, task={}, model_name={}, dataset_name={}, exp_id={}".format(
                ...
                if paras_type == "uniform":
                    ...
                    try:
                        search_space[name] = tune.uniform(
                            paras_dict[name]["lower"], paras_dict[name]["upper"]
                        )
                    except:
                        raise TypeError(
...
def hyper_parameter(
    ...
    experiment_config = ConfigParser(
        task, model_name, dataset_name, config_file=config_file, other_args=other_args
    )
    # exp_id
    ...
    logger.info(
        "Begin ray-tune, task={}, model_name={}, dataset_name={}, exp_id={}".format(
            str(task), str(model_name), str(dataset_name), str(exp_id)
        )
    )
    ...
    logger.info(
        "Best trial final validation loss: {}".format(best_trial.last_result["loss"])
    )
    # save best
