...
class ConfigParser(object):
    ...
    def __init__(
        ...
        self._parse_external_config(
            task, model, dataset, saved_model, train, other_args, hyper_config_dict
        )
    ...
    def _load_global_config(self):
        self.config["cache_file_folder"] = "./libcity/cache/dataset_cache/"

    def _parse_external_config(
        self,
        task,
        model,
        dataset,
    ...
    def _load_default_config(self):
        ...
        with open("./libcity/config/task_config.json", "r") as f:
            ...
            if self.config["model"] not in task_config["allowed_model"]:
                raise ValueError(
                    "task {} do not support model {}".format(
            ...
            if (
                self.config["task"] == "traj_loc_pred"
                and "traj_encoder" not in self.config
            ):
        ...
        default_file_list.append(
            "model/{}/{}.json".format(self.config["task"], self.config["model"])
        )
        # dataset
