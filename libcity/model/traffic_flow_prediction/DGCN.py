...
class SATT_3(nn.Module):
    ...
    def forward(self, seq):
        ...
        seq = (
            seq.permute(0, 1, 3, 2)
            .contiguous()
            .view(shape[0], shape[1] * 12, shape[3] // 12, shape[2])
        )
        ...
        f1 = (
            self.conv1(seq)
            .view(shape[0], self.c_in // 4, 4, shape[2], shape[3])
            .permute(0, 3, 1, 4, 2)
            .contiguous()
        )
        f2 = (
...
class SATT_2(nn.Module):
    def __init__(self, c_in, num_nodes):
        ...
        self.conv1 = Conv2d(
            c_in, c_in, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1), bias=False
        )
        self.conv2 = Conv2d(
            c_in, c_in, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1), bias=False
        )
...
class TATT_1(nn.Module):
    def __init__(self, c_in, num_nodes, tem_size, device):
        ...
        self.w = nn.Parameter(torch.rand(num_nodes, c_in), requires_grad=True).to(
            device
        )
        nn.init.xavier_uniform_(self.w)
        self.b = nn.Parameter(torch.zeros(tem_size, tem_size), requires_grad=True).to(
            device
        )
        self.v = nn.Parameter(torch.rand(tem_size, tem_size), requires_grad=True).to(
            device
        )
...
class ST_BLOCK_2(nn.Module):
    ...
    def forward(self, x, supports):
        ...
        hidden = (h, c)
        S_coef = (
            S_coef.permute(0, 2, 1, 3)
            .contiguous()
            .view(shape[0] * shape[2], shape[1], shape[3])
        )
...
class DGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.supports = (
            torch.tensor(scaled_laplacian(self.adj_mx))
            .type(torch.float32)
            .to(self.device)
        )
