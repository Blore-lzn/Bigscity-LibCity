...
class CRANN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.spatial_model = AttentionCNN(
            in_channels=self.input_window,
            out_channels=self.output_window,
            dim_x=self.dim_x,
            dim_y=self.dim_y,
        )
        self.temporal_encoder = EncoderLSTM(
            self.feature_dim, self.n_hidden_tem, device=self.device
        )
        self.temporal_decoder = BahdanauDecoder(self.n_hidden_tem, self.output_dim)
        self.mlp = MLP(
            n_inputs=self.len_inputs + self.n_ar * self.num_nodes,
            n_outputs=self.len_outputs,
            n_layers=self.n_layers_dns,
            n_hidden=self.n_hidden_dns,
        )
    ...
    def forward(self, batch):
        ...
        x = torch.cat(
            (
                y_time.unsqueeze(2),
                y_space.squeeze().view(-1, self.output_window, self.num_nodes),
                x_ext,
            ),
            dim=2,
        ).view(-1, self.len_inputs)
        x = torch.cat(
            (x, x_space[:, -self.n_ar :].view(-1, self.n_ar * self.num_nodes)), dim=1
        )
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, dim_x, dim_y, timesteps, method="hadamard"):
        ...
        self.weight = nn.Parameter(
            torch.FloatTensor(timesteps, dim_x * dim_y, dim_x * dim_y)
        )
        torch.nn.init.xavier_uniform_(self.weight)
        if method == "general":
            self.fc = nn.Linear(
                timesteps * (dim_x * dim_y) ** 2,
                timesteps * (dim_x * dim_y) ** 2,
                bias=False,
            )
        elif method == "concat":
...
class EncoderLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, n_layers=1, drop_prob=0, device="cuda"):
        ...
        self.lstm = nn.LSTM(
            input_size, hidden_size, n_layers, dropout=drop_prob, batch_first=True
        )
    ...
    def init_hidden(self, batch_size):
        return (
            torch.zeros(
                self.n_layers, batch_size, self.hidden_size, device=self.device
            ),
            torch.zeros(
                self.n_layers, batch_size, self.hidden_size, device=self.device
            ),
...
class BahdanauDecoder(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, n_layers=1, drop_prob=0.1):
        ...
        self.lstm = nn.LSTM(
            self.hidden_size + self.output_size, self.hidden_size, batch_first=True
        )
    ...
    def forward(self, inputs, hidden, encoder_outputs):
        ...
        x = torch.tanh(
            self.fc_hidden(hidden[0].view(-1, 1, self.hidden_size))
