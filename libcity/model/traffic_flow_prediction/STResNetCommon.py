...
class STResNetCommon(AbstractTrafficStateModel):
            ...
            OrderedDict(
                [
                    (
                        "conv1",
                        conv3x3(
                            in_channels=self.input_window * self.output_dim,
                            out_channels=64,
                        ),
                    ),
                    (
    ...
    def forward(self, batch):
        inputs = batch["X"][
            :, :, :, :, : self.output_dim
        ]  # (batch_size, input_window, len_row, len_column, output_dim)
        ...
        inputs = (
            inputs.contiguous()
            .view(
                -1, self.input_window * self.output_dim, self.len_row, self.len_column
            )
            .to(self.device)
        )
        ...
        if self.ext_dim > 0:
            ...
            external_output = external_output.view(
                -1, self.output_dim, self.len_row, self.len_column
            )
