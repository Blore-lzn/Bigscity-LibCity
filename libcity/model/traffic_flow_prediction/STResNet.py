...
class ResUnits(nn.Module):
    def __init__(self, residual_unit, nb_filter, repetations=1, bn=False):
        ...
        self.stacked_resunits = self.make_stack_resunits(
            residual_unit, nb_filter, repetations, bn
        )
...
class TrainableEltwiseLayer(nn.Module):
    ...
    def __init__(self, n, h, w, device):
        ...
        self.weights = nn.Parameter(
            torch.randn(1, n, h, w).to(device), requires_grad=True
...
class STResNet(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.output_dim = self.data_feature.get(
            "output_dim", 2
        ...
        if self.len_closeness > 0:
            self.c_way = self.make_one_way(
                in_channels=self.len_closeness * self.feature_dim
            )

        if self.len_period > 0:
            self.p_way = self.make_one_way(
                in_channels=self.len_period * self.feature_dim
            )

        if self.len_trend > 0:
            self.t_way = self.make_one_way(
                in_channels=self.len_trend * self.feature_dim
            )
                ...
                OrderedDict(
                    [
                        ...
                        (
                            ...
                            nn.Linear(
                                10,
                                self.output_dim * self.len_row * self.len_column,
                                bias=True,
                            ),
                        ),
                        ("relu2", nn.ReLU()),
                ...
                [
                    ...
                    (
                        ...
                        ResUnits(
                            ResidualUnit,
                            nb_filter=64,
                            repetations=self.nb_residual_unit,
                            bn=self.bn,
                        ),
                    ),
                    ("relu", nn.ReLU()),
    ...
    def forward(self, batch):
        inputs = batch[
            "X"
        ]  # (batch_size, T_c+T_p+T_t, len_row, len_column, feature_dim)
        ...
        if self.len_closeness > 0:
            ...
            input_c = input_c.view(
                -1, self.len_closeness * self.feature_dim, self.len_row, self.len_column
            )
        ...
        if self.len_period > 0:
            ...
            input_p = input_p.view(
                -1, self.len_period * self.feature_dim, self.len_row, self.len_column
            )
        ...
        if self.len_trend > 0:
            ...
            input_t = input_t.view(
                -1, self.len_trend * self.feature_dim, self.len_row, self.len_column
            )
        ...
        if self.ext_dim > 0:
            ...
            external_output = external_output.view(
                -1, self.feature_dim, self.len_row, self.len_column
            )
            main_output += external_output
        main_output = self.tanh(main_output)
        main_output = main_output.view(
            batch_size, 1, len_row, len_column, self.output_dim
        )
        return main_output
