...
class STNN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.mode is None or self.mode == "refine":
            self.relations = torch.cat(
                (torch.eye(self.num_nodes).unsqueeze(1), relations), 1
            )
        elif self.mode == "discover":
            self.relations = torch.cat(
                (
                    torch.eye(self.num_nodes).unsqueeze(1),
                    torch.ones(self.num_nodes, 1, self.num_nodes).to(device),
                ),
                1,
            )
        ...
        self.factors = nn.Parameter(
            torch.Tensor(self.input_window, self.num_nodes, self.feature_dim)
        )
        self.sigmo = nn.Sigmoid()
        self.ffunc = nn.Linear(
            self.input_window * self.num_nodes * self.feature_dim,
            2 * self.input_window * self.num_nodes * self.feature_dim,
        )

        self.dynamic = MLP(
            self.feature_dim * self.nr, nhid, self.feature_dim, nlayers, dropout_d
        )
        self.decoder = nn.Linear(
            self.feature_dim * self.input_window,
        ...
        if self.mode == "refine":
            ...
            self.rel_weights = nn.Parameter(
                torch.Tensor(self.relations.sum().item() - self.num_nodes)
            )
        elif self.mode == "discover":
            self.rel_weights = nn.Parameter(
                torch.Tensor(self.num_nodes, 1, self.num_nodes)
            )
        # init
        self._init_weights()
    ...
    def get_relations(self):
        if self.mode is None:
            ...
            if self.mode == "refine":
                intra = (
                    self.rel_weights.new(self.num_nodes, self.num_nodes)
                    .copy_(self.relations[:, 0])
                    .unsqueeze(1)
                )
                inter = self.rel_weights.new_zeros(
                    self.num_nodes, self.nr - 1, self.num_nodes
                )
    ...
    def predict(self, batch):
        ...
        # step one:Xt to Zt
        x_stepone = self.ffunc(
            x.view(x_size[0], self.input_window * self.num_nodes * self.feature_dim)
        )
        x_steptwo = self.sigmo(
            x_stepone.view(
                x_size[0], self.input_window, self.num_nodes, self.feature_dim * 2
            )
        )
        z_inf = self.drop(
            self.factors[
                (x_steptwo[:, :, :, 0] * 11).ceil().long(),
                (x_steptwo[:, :, :, 1] * 40).ceil().long(),
            ]
        )
