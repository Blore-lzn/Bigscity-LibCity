...
class MSTGCNSubmodule(nn.Module):
    def __init__(
        ...
        self.fusionlayer = FusionLayer(
            output_window, num_of_vertices, output_dim, device
        )
...
class MSTGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.len_closeness > 0:
            self.hours_MSTGCN_submodule = MSTGCNSubmodule(
                self.device,
                self.nb_block,
                self.feature_dim,
                self.K,
                self.nb_chev_filter,
                self.nb_time_filter,
                self.len_closeness // self.output_window,
                self.cheb_polynomials,
                self.output_window,
                self.output_dim,
                self.num_nodes,
        ...
        if self.len_period > 0:
            self.days_MSTGCN_submodule = MSTGCNSubmodule(
                self.device,
                self.nb_block,
                self.feature_dim,
                self.K,
                self.nb_chev_filter,
                self.nb_time_filter,
                self.len_period // self.output_window,
                self.cheb_polynomials,
                self.output_window,
                self.output_dim,
                self.num_nodes,
        ...
        if self.len_trend > 0:
            self.weeks_MSTGCN_submodule = MSTGCNSubmodule(
                self.device,
                self.nb_block,
                self.feature_dim,
                self.K,
                self.nb_chev_filter,
                self.nb_time_filter,
                self.len_trend // self.output_window,
                self.cheb_polynomials,
                self.output_window,
                self.output_dim,
                self.num_nodes,
    ...
    def forward(self, batch):
        ...
        if self.len_closeness > 0:
            ...
            output_hours = self.hours_MSTGCN_submodule(
        ...
        if self.len_trend > 0:
            ...
            output_weeks = self.weeks_MSTGCN_submodule(
                x[:, begin_index:end_index, :, :]
            )
            output += output_weeks
