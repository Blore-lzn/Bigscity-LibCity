...
class CBAAttention(nn.Module):
    ...
    def forward(self, inputs):
        assert (
            ...
            and len(inputs) == 2
            and inputs[0].shape[-1] == self.att_size
            and inputs[1].shape[-1] == self.query_dim
        )
        memory, query = inputs
...
class STDN(AbstractTrafficStateModel):
    ...
    def _init_model(self):
        ...
        # lstm
        self.lstm = nn.LSTM(
            input_size=self.feature_vec_len + self.cnn_flat_size,
        ...
        self.att_flow_gate_1st = nn.ModuleList(
            [
                nn.ModuleList(
                    [nn.Sigmoid().to(self.device) for j in range(self.att_lstm_seq_len)]
                )
                for i in range(self.att_lstm_num)
            ]
        )
                ...
                nn.ModuleList(
                    [
                        nn.Sequential(
                            nn.Flatten(),
                            nn.Linear(
                                64 * self.nbhd_size * self.nbhd_size, self.cnn_flat_size
                            ),
                            nn.ReLU(),
                        ).to(self.device)
                        for j in range(self.att_lstm_seq_len)
        ...
        self.att_high_level = nn.LSTM(
            input_size=self.lstm_out_size,
            hidden_size=self.lstm_out_size,
            batch_first=True,
            dropout=0.1,
        ).to(self.device)
        ...
        self.lstm_all = nn.Linear(
            self.lstm_out_size + self.lstm_out_size, self.output_dim
        ).to(self.device)
    ...
    def forward(self, batch):
        ...
        for att in range(self.att_lstm_num):
            att_nbhd_inputs.append(
                flatten_att_nbhd_inputs[
                    att * self.att_lstm_seq_len : (att + 1) * self.att_lstm_seq_len
                ]
            )
        ...
        nbhd_inputs = [
            nbhd_inputs[ts].permute(0, 3, 1, 2) for ts in range(self.lstm_seq_len)
        ]
        flow_inputs = [
            flow_inputs[ts].permute(0, 3, 1, 2) for ts in range(self.lstm_seq_len)
        ]

        nbhd_convs = [
            self.nbhd_cnns_1st[ts](nbhd_inputs[ts]) for ts in range(self.lstm_seq_len)
        ]
        flow_convs = [
            self.flow_cnns_1st[ts](flow_inputs[ts]) for ts in range(self.lstm_seq_len)
        ]

        nbhd_convs = [
            nbhd_convs[ts] * flow_convs[ts] for ts in range(self.lstm_seq_len)
        ]

        nbhd_convs = [
            self.nbhd_cnns_2nd[ts](nbhd_convs[ts]) for ts in range(self.lstm_seq_len)
        ]
        flow_convs = [
            self.flow_cnns_2nd[ts](flow_inputs[ts]) for ts in range(self.lstm_seq_len)
        ]
        ...
        nbhd_convs = [
            self.nbhd_cnns_3rd[ts](nbhd_convs[ts]) for ts in range(self.lstm_seq_len)
        ]
        flow_convs = [
            self.flow_cnns_3rd[ts](flow_inputs[ts]) for ts in range(self.lstm_seq_len)
        ]
        ...
        nbhd_vecs = [
            self.nbhd_vecs[ts](nbhd_convs[ts]) for ts in range(self.lstm_seq_len)
        ]
        ...
        att_nbhd_inputs = [
            [
                att_nbhd_inputs[att][ts].permute(0, 3, 1, 2)
        ...
        att_flow_inputs = [
            [
                att_flow_inputs[att][ts].permute(0, 3, 1, 2)
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        ...
        att_nbhd_convs = [
            [
                self.att_nbhd_cnns_1st[att][ts](att_nbhd_inputs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        att_flow_convs = [
            [
                self.att_flow_cnns_1st[att][ts](att_flow_inputs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        att_flow_gates = [
            [
                self.att_flow_gate_1st[att][ts](att_flow_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        ...
        att_nbhd_convs = [
            [
                self.att_nbhd_cnns_2nd[att][ts](att_nbhd_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        att_flow_convs = [
            [
                self.att_flow_cnns_2nd[att][ts](att_flow_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        att_flow_gates = [
            [
                self.att_flow_gate_2nd[att][ts](att_flow_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        ...
        att_nbhd_convs = [
            [
                self.att_nbhd_cnns_3rd[att][ts](att_nbhd_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        att_flow_convs = [
            [
                self.att_flow_cnns_3rd[att][ts](att_flow_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        att_flow_gates = [
            [
                self.att_flow_gate_3rd[att][ts](att_flow_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        ...
        att_nbhd_vecs = [
            [
                self.att_nbhd_vecs[att][ts](att_nbhd_convs[att][ts])
                for ts in range(self.att_lstm_seq_len)
            ]
            for att in range(self.att_lstm_num)
        ]
        ...
        att_nbhd_vec = [
            torch.stack(att_nbhd_vecs[att], dim=1) for att in range(self.att_lstm_num)
        ]
        att_lstm_input = [
            torch.cat([att_lstm_inputs[att], att_nbhd_vec[att]], dim=-1)
            for att in range(self.att_lstm_num)
        ]
        att_lstms = [
            self.att_lstms[att](att_lstm_input[att])[0]
            for att in range(self.att_lstm_num)
        ]
        # shape=[att_lstm_num, (batch_size, lstm_out_size)]

        att_low_level = [
            self.att_low_level[att]([att_lstms[att], lstm])
            for att in range(self.att_lstm_num)
        ]
    ...
    def predict(self, batch):
        return self.forward(batch).reshape(
            -1, self.output_window, self.len_row, self.len_column, self.output_dim
        )
