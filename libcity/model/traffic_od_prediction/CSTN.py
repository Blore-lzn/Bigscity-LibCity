...
class CNN(nn.Module):
    def __init__(self, height, width, n_layers):
        ...
        for i in range(1, self.n_layers):
            ...
            self.conv.append(
                nn.Conv2d(
                    in_channels=16, out_channels=16, kernel_size=(3, 3), padding=(1, 1)
                )
            )
        self.relu = nn.ReLU()
        self.embed = nn.Sequential(
            nn.Conv2d(
                in_channels=self.height * self.width * 16,
                out_channels=32,
                kernel_size=(1, 1),
            ),
            nn.ReLU(),
...
class TEC(nn.Module):
    def __init__(self, c_lt):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=c_lt, kernel_size=(1, 1)), nn.ReLU()
        )
...
class CSTN(AbstractTrafficStateModel):
    ...
    def forward(self, batch):
        ...
        # x : (B, H * W, H, W)
        x = x.reshape(
            (self.batch_size, 1, self.height, self.width, self.height, self.width, 1)
        )
        # x : (B, 1, H, W, H, W, 1)
        return x
