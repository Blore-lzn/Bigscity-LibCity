...
from libcity.utils.GPS_utils import (
    radian2angle,
    R_EARTH,
    angle2radian,
    dist,
...
class STMatching(AbstractTraditionModel):
    """
    ST-Matching
    """
    ...
    def __init__(self, config, data_feature):
        ...
        self.k = config.get(
            "k", 5
        )  # maximum number of candidates of any sampling points
        self.r = config.get("r", 100)  # radius of road segment candidate
        self.mu = config.get("mu", 0)
        self.sigma = config.get("sigma", 20)
    ...
    def run(self, data):
        ...
        for usr_id, usr_value in trajectory.items():
            ...
            for traj_id, value in usr_value.items():
                self._logger.info(
                    "begin map matching, usr_id:%d traj_id:%d" % (usr_id, traj_id)
                )
                self.traj_id = traj_id
                self.trajectory = value
                self._run_one_tra()
                self._logger.info(
                    "finish map matching, usr_id:%d traj_id:%d" % (usr_id, traj_id)
                )
        return self.res_dct
    ...
    def _get_candidates(self):
        ...
        for i in range(traj_lon_lat.shape[0]):
            ...
            for j in self.rd_nwk.edges:
                ...
                if (
                    lat - self.lat_r <= lat_origin <= lat + self.lat_r
                    ...
                    lat - self.lat_r
                    <= lat_origin / 2 + lat_dest / 2
                    <= lat + self.lat_r
                    and lon - self.lon_r
                    <= lon_origin / 2 + lon_dest / 2
                    <= lon + self.lon_r
                ):
                    candidate_i.add((origin, dest))
                ...
                for edge_k, dct_k in self.candidates[k].items():
                    ...
                    try:
                        if dct_j["node"] is not None and dct_k["node"] is not None:
                            ...
                            nd2_origin = edge_k[0]
                            lon, lat = (
                                self.rd_nwk.nodes[nd2_origin]["lon"],
                                self.rd_nwk.nodes[nd2_origin]["lat"],
                            )
                            ...
                            if edge_j[1] == dct_j["edge"]:
                                path_len += (
                                    self.rd_nwk[edge_j[0]][edge_j[1]]["distance"] * 2
                                )
                            result = d / path_len
                            ...
                            lon, lat = (
                                self.rd_nwk.nodes[nd1_destination]["lon"],
                            ...
                            if edge_k[1] == dct_k["node"]:
                                path_len += (
                                    self.rd_nwk[edge_k[0]][edge_k[1]]["distance"] * 2
                                )
                            result = d / path_len
                        else:
                            if (
                                edge_j == edge_k
                                and math.fabs(brng_edge_j - brng_jk) < 90
                            ):
                                result = 1
                            else:
                                nd1_destination = edge_j[1]
    ...
    def _find_matched_sequence(self):
        ...
        for i in range(len(pre) - 1, -1, -1):
            if e is None:
                if pre[i] is not None:
                    ...
                    e = max(
                        self.candidates[i], key=lambda k: self.candidates[i][k]["score"]
                    )
                    res_lst.append(e)
                else:
        ...
        # to geo_id
        res_lst_rel = np.array(
            list(
                map(
                    lambda x: self.rd_nwk.edges[x]["geo_id"] if x is not None else None,
                    res_lst,
                )
            )
        )
