...
class GWNET(AbstractTrafficStateModel):
                ...
                np.round(
                    np.log(
                        (
                            (
                                (self.input_window - 1)
                                / (self.blocks * (self.kernel_size - 1))
                            )
                            + 1
                        )
                    )
        ...
        if self.gcn_bool and self.addaptadj:
            if self.aptinit is None:
                ...
                self.nodevec1 = nn.Parameter(
                    torch.randn(self.num_nodes, 10).to(self.device), requires_grad=True
                ).to(self.device)
                self.nodevec2 = nn.Parameter(
                    torch.randn(10, self.num_nodes).to(self.device), requires_grad=True
                ).to(self.device)
                self.supports_len += 1
            else:
                if self.supports is None:
    ...
    def forward(self, batch):
        ...
        inputs = nn.functional.pad(
            inputs, (1, 0, 0, 0)
        )  # (batch_size, feature_dim, num_nodes, input_window+1)
    ...
    def cal_adj(self, adjtype):
        if adjtype == "scalap":
            ...
            self.adj_mx = [
                calculate_normalized_laplacian(self.adj_mx).astype(np.float32).todense()
            ]
