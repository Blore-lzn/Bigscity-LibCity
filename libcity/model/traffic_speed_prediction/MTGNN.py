...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs, idx):
        if self.elementwise_affine:
            ...
            return F.layer_norm(
                inputs, tuple(inputs.shape[1:]), self.weight, self.bias, self.eps
            )
...
class MTGNN(AbstractTrafficStateModel):
    ...
    def forward(self, batch, idx=None):
        ...
        assert (
            inputs.size(3) == self.input_window
        ), "input sequence length not equal to preset sequence length"

        if self.input_window < self.receptive_field:
            inputs = nn.functional.pad(
                inputs, (self.receptive_field - self.input_window, 0, 0, 0)
            )
