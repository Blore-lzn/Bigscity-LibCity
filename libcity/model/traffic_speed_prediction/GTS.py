...
class DCGRUCell(nn.Module):
    ...
    def _calculate_random_walk_matrix(self, adj_mx):
        ...
        d_inv = 1.0 / d
        d_inv = torch.where(
            torch.isinf(d_inv), torch.zeros(d_inv.shape).to(self._device), d_inv
        )
...
class GTS(AbstractTrafficStateModel, Seq2SeqAttrs):
    ...
    def _get_x_y_in_correct_dims(self, x, y):
        ...
        y = (
            y[..., : self.output_dim]
            .contiguous()
            .view(self.horizon, batch_size, self.num_nodes * self.output_dim)
        )
        return x, y
    ...
    def forward(self, batch, batches_seen=None):
        ...
        # 图结构的推断过程
        x = self.node_feas.transpose(1, 0).view(
            self.num_nodes, 1, -1
        )  # [207, 1, 24000]
    ...
    def calculate_loss(self, batch, batches_seen=None):
        ...
        if epoch < self.epoch_use_regularization:
            ...
            true_label = self.adj_mx.view(mid_output.shape[0] * mid_output.shape[1]).to(
                self.device
            )
