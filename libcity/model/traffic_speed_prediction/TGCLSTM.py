...
class FilterLinear(nn.Module):
    def __init__(
        ...
        self.filter_square_matrix = Variable(
            filter_square_matrix.repeat(output_dim, input_dim).to(device),
            requires_grad=False,
        )
...
class TGCLSTM(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        for i in range(self.K):
            ...
            if config.get("Clamp_A", True):
                ...
                adj_temp = torch.clamp(adj_temp, max=1.0)
            if self.dataset_class == "TGCLSTMDataset":
                self.A_list.append(
                    torch.mul(
                        adj_temp,
                        torch.Tensor(data_feature["FFR"][self.back_length]).to(
                            self.device
                        ),
                    )
                )
        ...
        self.Neighbor_weight = Parameter(
            torch.FloatTensor(self.out_features, self.out_features).to(self.device)
        )
    ...
    def step(self, step_input, hidden_state, cell_state):
        ...
        for i in range(1, self.K):
            gc = torch.cat(
                (gc, self.gc_list[i](x)), 1
            )  # [batch_size, out_features * K]

        combined = torch.cat(
            (gc, hidden_state), 1
        )  # [batch_size, out_features * (K+1)]
        ...
        nc = torch.matmul(
            ...
            torch.mul(
                Variable(
                    self.A_list[-1].repeat(self.output_dim, self.output_dim),
                    requires_grad=False,
                ).to(self.device),
    ...
    def forward(self, batch):
        ...
        for i in range(time_step):
            step_input = torch.squeeze(
                torch.transpose(inputs[:, i : i + 1, :, :], 2, 3)
            ).reshape(batch_size, -1)
            hidden_state, cell_state, gc = self.step(
                step_input, hidden_state, cell_state
            )
            ...
            if outputs is None:
                ...
                outputs = torch.cat(
                    (outputs, hidden_state.unsqueeze(1)), 1
                )  # [batch_size, input_window, out_features]
        output = torch.transpose(
            torch.squeeze(outputs[:, -1, :]).reshape(
    ...
    def predict(self, batch):
        ...
        y_preds = torch.cat(
            y_preds, dim=1
        )  # [batch_size, output_window, batch_size, output_dim]
        return y_preds

    def init_hidden(self, batch_size):
        hidden_state = Variable(
            torch.zeros(batch_size, self.out_features).to(self.device)
        )
        cell_state = Variable(
            torch.zeros(batch_size, self.out_features).to(self.device)
        )
