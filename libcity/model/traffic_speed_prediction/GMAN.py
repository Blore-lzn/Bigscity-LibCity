...
class FC(nn.Module):  # is_training: self.training
    def __init__(
        self, input_dims, units, activations, bn, bn_decay, device, use_bias=True
    ):
    ...
    def _init_layers(self):
        ...
        for num_unit, activation in zip(units, activations):
            if self.use_bias:
                basic_conv2d = nn.Conv2d(
                    input_dims, num_unit, (1, 1), stride=1, padding=0, bias=True
                )
                nn.init.constant_(basic_conv2d.bias, 0)
            else:
                basic_conv2d = nn.Conv2d(
                    input_dims, num_unit, (1, 1), stride=1, padding=0, bias=False
                )
            ...
            if activation is not None:
                if self.bn:
                    ...
                    basic_batch_norm = nn.BatchNorm2d(
                        num_unit, eps=1e-3, momentum=decay
                    )
...
class STAttBlock(nn.Module):
    def __init__(self, K, D, bn, bn_decay, device, mask=True):
        ...
        self.sp_att = SpatialAttention(
            K=self.K, D=self.D, bn=self.bn, bn_decay=self.bn_decay, device=self.device
        )
        self.temp_att = TemporalAttention(
            K=self.K, D=self.D, bn=self.bn, bn_decay=self.bn_decay, device=self.device
        )
        self.gated_fusion = GatedFusion(
            D=self.D, bn=self.bn, bn_decay=self.bn_decay, device=self.device
        )
...
class TransformAttention(nn.Module):
    ...
    def forward(self, x, ste1, ste2):
        ...
        attention = torch.softmax(
            attention, dim=-1
        )  # (K*batch_size, num_nodes, output_length, input_length)
...
class STEmbedding(nn.Module):
    def __init__(self, T, D, bn, bn_decay, add_day_in_week, device):
        ...
        self.TE_fc = FC(
            input_dims=7 + self.T if add_day_in_week else self.T,
            units=[self.D, self.D],
            activations=[nn.ReLU, None],
            bn=self.bn,
            bn_decay=self.bn_decay,
...
class GMAN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        #                           device=self.device)
        self.trans_att = TransformAttention(
            K=self.K, D=self.D, bn=self.bn, bn_decay=self.bn_decay, device=self.device
        )
    ...
    def forward(self, batch):
        ...
        _timeofday = _timeofday.to(
            int
        )  # (batch_size, input_length+output_length, num_nodes, 1)
        _timeofday = _timeofday[
            :, :, 0, :
        ]  # (batch_size, input_length+output_length, 1)
        timeofday = torch.zeros(
            (_timeofday.size(0), _timeofday.size(1), self.T), device=self.device
        ).long()
        timeofday.scatter_(
            dim=2,
            index=_timeofday.long(),
            src=torch.ones(timeofday.shape, device=self.device).long(),
        )
        if self.add_day_in_week:
            _dayofweek = TE[:, :, :, 1:]
            _dayofweek = _dayofweek.to(
                int
            )  # (batch_size, input_length+output_length, num_nodes, 7)
            dayofweek = _dayofweek[
                :, :, 0, :
            ]  # (batch_size, input_length+output_length, 7)
            TE = torch.cat((dayofweek, timeofday), dim=2).type(torch.FloatTensor)
        else:
            TE = timeofday.type(torch.FloatTensor)
        TE = TE.unsqueeze(2).to(
            device=self.device
        )  # (batch_size, input_length+output_length, 1, 7+T or T)
