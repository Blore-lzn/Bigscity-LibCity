...
def softmax(
    x: torch.Tensor, index: torch.Tensor, num_nodes: Optional[int] = None, dim: int = 0
):
    N = maybe_num_nodes(index, num_nodes)
    x_max = scatter(x, index, dim, dim_size=N, reduce="max").index_select(dim, index)
    out = (x - x_max).exp()
    out_sum = scatter(out, index, dim, dim_size=N, reduce="sum").index_select(
        dim, index
    )
    return out / out_sum
...
class STAGGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        # 对数据集预处理得到的空间邻接边集
        self.edge_index = self.data_feature.get(
            "edge_index", torch.tensor([[], []], dtype=torch.long)
        )  # 空间邻接边
        # 对数据集预处理得到的语义邻接边集
        self.dtw_edge_index = self.data_feature.get(
            "dtw_edge_index", torch.tensor([[], []], dtype=torch.long)
        )  # 语义邻接边
...
class STAGGCNModel(nn.Module):
    def __init__(
        ...
        self.output_linear = nn.Linear(
            in_features=self.pred_len_raw, out_features=self.pred_len
        )
...
class LearnedGCN(nn.Module):
    ...
    def reset_parameters(self):
        stdv = 1.0 / math.sqrt(self.source_embed.size(0))
        self.source_embed.data.uniform_(-stdv, stdv)
        self.target_embed.data.uniform_(-stdv, stdv)

    def forward(self, input):
        learned_matrix = F.softmax(
            F.relu(torch.mm(self.source_embed, self.target_embed)), dim=1
        )
...
class GATConv(nn.Module):
    def __init__(
        ...
        self.linear = nn.Linear(
            self.in_channels, self.attn_heads * self.out_channels, bias=False
        )
...
class STCell(nn.Module):
    def __init__(
        ...
        self.seq_linear = nn.Linear(
            in_features=self.input_dim * seq_len, out_features=self.input_dim * seq_len
        )
        ...
        if choice[0] == 1:
            ...
            self.self_attn = nn.MultiheadAttention(
                embed_dim=node_num * input_dim, num_heads=attn_head
            )
            ...
            self.tlinear = nn.Linear(
                in_features=self.output_dim * self.tcn_dim[-1] * self.seq_len,
                out_features=self.output_dim * self.graph_dim,
            )

        if choice[1] == 1:
            print("[SP]")
            self.sp_origin = nn.Linear(
                in_features=self.input_dim * seq_len,
                out_features=self.output_dim * graph_dim,
            )
            self.sp_gconv1 = GATConv(
                self.input_dim * seq_len,
                self.output_dim * graph_dim,
                heads=3,
                concat=False,
            )
            self.sp_gconv2 = GATConv(
                self.output_dim * graph_dim,
                self.output_dim * graph_dim,
                heads=3,
                concat=False,
            )
            self.sp_gconv3 = GATConv(
                self.output_dim * graph_dim,
                self.output_dim * graph_dim,
                heads=3,
                concat=False,
            )
            self.sp_gconv4 = GATConv(
                self.output_dim * graph_dim,
                self.output_dim * graph_dim,
                heads=1,
                concat=False,
            ...
            self.sp_linear_1 = nn.Linear(
                self.input_dim * seq_len, self.output_dim * self.graph_dim
            )
            self.sp_linear_2 = nn.Linear(
                self.output_dim * self.graph_dim, self.output_dim * self.graph_dim
            )
            self.sp_linear_3 = nn.Linear(
                self.output_dim * self.graph_dim, self.output_dim * self.graph_dim
            )
            self.sp_linear_4 = nn.Linear(
                self.output_dim * self.graph_dim, self.output_dim * self.graph_dim
            )
        ...
        if choice[2] == 1:
            print("[DTW]")
            self.dtw_origin = nn.Linear(
                in_features=self.input_dim * seq_len,
                out_features=self.output_dim * graph_dim,
            )
            self.dtw_gconv1 = GATConv(
                self.input_dim * seq_len,
                self.output_dim * graph_dim,
                heads=3,
                concat=False,
            )
            self.dtw_gconv2 = GATConv(
                self.output_dim * graph_dim,
                self.output_dim * graph_dim,
                heads=3,
                concat=False,
            )
            self.dtw_gconv3 = GATConv(
                self.output_dim * graph_dim,
                self.output_dim * graph_dim,
                heads=3,
                concat=False,
            )
            self.dtw_gconv4 = GATConv(
                self.output_dim * graph_dim,
                self.output_dim * graph_dim,
                heads=3,
                concat=False,
            ...
            self.dtw_linear_1 = nn.Linear(
                self.input_dim * self.seq_len, self.output_dim * self.graph_dim
            )
            self.dtw_linear_2 = nn.Linear(
                self.output_dim * self.graph_dim, self.output_dim * self.graph_dim
            )
            self.dtw_linear_3 = nn.Linear(
                self.output_dim * self.graph_dim, self.output_dim * self.graph_dim
            )
            self.dtw_linear_4 = nn.Linear(
                self.output_dim * self.graph_dim, self.output_dim * self.graph_dim
            )
    ...
    def forward(self, x, edge_index, dtw_edge_index):
        ...
        if self.choice[0] == 1:
            ...
            attn_input = (
                x.permute(1, 0, 3, 2).reshape(self.seq_len, batch_size, -1).contiguous()
            )
            # [seq_len, batch_size, input_dim*num_nodes]
            ...
            attn_output = (
                attn_output.reshape(
                    self.seq_len, batch_size, self.input_dim, self.node_num
                )
                .permute(1, 3, 2, 0)
            ...
            # [batch_size*num_nodes, output_dim*self.tcn_dim[-1]*seq_len]
            tcn_output = torch.reshape(
                tcn_output, (-1, self.output_dim * self.tcn_dim[-1] * self.seq_len)
            )
            # [batch_size*num_nodes, output_dim*self.graph_dim]
            tcn_output = self.tlinear(tcn_output)
            # [batch_size, num_nodes, output_dim, self.graph_dim]
            tcn_output = torch.reshape(
                tcn_output, (batch_size, self.node_num, self.output_dim, self.graph_dim)
            )
        ...
        if self.choice[1] == 1:
            ...
            # [num_nodes, num_nodes]
            sp_learned_matrix = F.softmax(
                F.relu(torch.mm(self.sp_source_embed, self.sp_target_embed)), dim=1
            )
            ...
            # [batch_size, num_nodes, input_dim*seq_len]
            adp_input_1 = torch.reshape(
                sp_gout_0, (-1, self.node_num, self.input_dim * self.seq_len)
            )
            # [batch_size, num_nodes, output_dim*graph_dim]
            sp_adp_1 = self.sp_linear_1(
                sp_learned_matrix.matmul(F.dropout(adp_input_1, p=0.1))
            )
            # [batch_size*num_nodes, output_dim*graph_dim]
            ...
            # [batch_size*num_nodes, output_dim*graph_dim]
            sp_output_1 = torch.tanh(sp_gout_1) * torch.sigmoid(
                sp_adp_1
            ) + sp_origin * (1 - torch.sigmoid(sp_adp_1))

            # [batch_size*num_nodes, output_dim*graph_dim]
            sp_gout_2 = self.sp_gconv2(torch.tanh(sp_output_1), edge_index)
            # [batch_size, num_nodes, output_dim*graph_dim]
            adp_input_2 = torch.reshape(
                torch.tanh(sp_output_1),
                (-1, self.node_num, self.output_dim * self.graph_dim),
            )
            # [batch_size, num_nodes, output_dim*graph_dim]
            sp_adp_2 = self.sp_linear_2(
                sp_learned_matrix.matmul(F.dropout(adp_input_2, p=0.1))
            )
            # [batch_size*num_nodes, output_dim*graph_dim]
            sp_adp_2 = torch.reshape(sp_adp_2, (-1, self.output_dim * self.graph_dim))
            # [batch_size*num_nodes, output_dim*graph_dim]
            sp_output_2 = F.leaky_relu(sp_gout_2) * torch.sigmoid(
                sp_adp_2
            ...
            # [batch_size, num_nodes, output_dim*graph_dim]
            adp_input_3 = torch.reshape(
                F.relu(sp_output_2),
                (-1, self.node_num, self.output_dim * self.graph_dim),
            )
            # [batch_size, num_nodes, output_dim*graph_dim]
            sp_adp_3 = self.sp_linear_3(
                sp_learned_matrix.matmul(F.dropout(adp_input_3, p=0.1))
            )
            # [batch_size*num_nodes, output_dim*graph_dim]
            sp_adp_3 = torch.reshape(sp_adp_3, (-1, self.output_dim * self.graph_dim))
            # [batch_size*num_nodes, output_dim*graph_dim]
            sp_output_3 = F.relu(sp_gout_3) * torch.sigmoid(sp_adp_3) + sp_output_2 * (
                1 - torch.sigmoid(sp_adp_3)
            )

            sp_gout_4 = self.sp_gconv4(F.relu(sp_output_3), edge_index)
            adp_input_4 = torch.reshape(
                F.relu(sp_output_3),
                (-1, self.node_num, self.output_dim * self.graph_dim),
            )
            sp_adp_4 = self.sp_linear_4(
                sp_learned_matrix.matmul(F.dropout(adp_input_4, p=0.1))
            )
            sp_adp_4 = torch.reshape(sp_adp_4, (-1, self.output_dim * self.graph_dim))
            # [batch_size*num_nodes, output_dim*graph_dim]
            sp_output_4 = F.relu(sp_gout_4) * torch.sigmoid(sp_adp_4) + sp_output_3 * (
                1 - torch.sigmoid(sp_adp_4)
            )
            ...
            # [batch_size, num_nodes, output_dim, graph_dim]
            sp_output = torch.reshape(
                sp_output_4,
                (batch_size, self.node_num, self.output_dim, self.graph_dim),
            )
        ...
        if self.choice[2] == 1:
            ...
            dtw_learned_matrix = F.softmax(
                F.relu(torch.mm(self.dtw_source_embed, self.dtw_target_embed)), dim=1
            )

            dtw_gout_1 = self.dtw_gconv1(dtw_gout_0, dtw_edge_index)
            adp_input_1 = torch.reshape(
                dtw_gout_0, (-1, self.node_num, self.input_dim * self.seq_len)
            )
            dtw_adp_1 = self.dtw_linear_1(
                dtw_learned_matrix.matmul(F.dropout(adp_input_1, p=0.1))
            )
            dtw_adp_1 = torch.reshape(dtw_adp_1, (-1, self.output_dim * self.graph_dim))
            dtw_origin = self.dtw_origin(dtw_gout_0)
            dtw_output_1 = torch.tanh(dtw_gout_1) * torch.sigmoid(
                dtw_adp_1
            ) + dtw_origin * (1 - torch.sigmoid(dtw_adp_1))

            dtw_gout_2 = self.dtw_gconv2(torch.tanh(dtw_output_1), dtw_edge_index)
            adp_input_2 = torch.reshape(
                torch.tanh(dtw_output_1),
                (-1, self.node_num, self.output_dim * self.graph_dim),
            )
            dtw_adp_2 = self.dtw_linear_2(
                dtw_learned_matrix.matmul(F.dropout(adp_input_2, p=0.1))
            )
            dtw_adp_2 = torch.reshape(dtw_adp_2, (-1, self.output_dim * self.graph_dim))
            dtw_output_2 = F.leaky_relu(dtw_gout_2) * torch.sigmoid(
                dtw_adp_2
            ...
            dtw_adp_3 = self.dtw_linear_3(
                dtw_learned_matrix.matmul(F.dropout(adp_input_3, p=0.1))
            )
            dtw_adp_3 = torch.reshape(dtw_adp_3, (-1, self.output_dim * self.graph_dim))
            dtw_output_3 = F.relu(dtw_gout_3) * torch.sigmoid(
                dtw_adp_3
            ) + dtw_output_2 * (1 - torch.sigmoid(dtw_adp_3))
            ...
            dtw_adp_4 = self.dtw_linear_4(
                dtw_learned_matrix.matmul(F.dropout(adp_input_4, p=0.1))
            )
            dtw_adp_4 = torch.reshape(dtw_adp_4, (-1, self.output_dim * self.graph_dim))
            # [batch_size*num_nodes, output_dim*graph_dim]
            dtw_output_4 = F.relu(dtw_gout_4) * torch.sigmoid(
                dtw_adp_4
            ) + dtw_output_3 * (1 - torch.sigmoid(dtw_adp_4))
