...
class SANN(nn.Module):
    def __init__(
        self, n_inp, n_out, t_inp, t_out, n_points, past_t, hidden_dim, dropout
    ):
        ...
        # Convolutional layer
        self.conv_block = AgnosticConvBlock(
            n_inp, n_points, past_t, hidden_dim, num_conv=1
        )
        self.convT = nn.ConvTranspose2d(
            in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=(1, n_points)
        )
        # Regressor layer
    ...
    def __init__(self, n_inp, n_points, past_t, hidden_dim, num_conv):
        ...
        layers = [
            nn.Conv2d(
                ...
                kernel_size=(past_t, n_points),
                bias=True,
            ),
            nn.BatchNorm2d(
                num_features=hidden_dim, affine=True, track_running_stats=True
            ),
            nn.ReLU(),
        ]
