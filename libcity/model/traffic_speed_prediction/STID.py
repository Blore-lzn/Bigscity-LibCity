...
class STID(AbstractTrafficStateModel):
    ...
    def __init__(self, config, data_feature):
        ...
        assert (
            24 * 60 * 60
        ) % self.time_intervals == 0, "time_of_day_size should be Int"
        ...
        if self.if_spatial:
            self.node_emb = nn.Parameter(
                torch.empty(self.num_nodes, self.spatial_emb_dim)
            )
        ...
        # encoding
        self.hidden_dim = (
            self.time_series_emb_dim
            + self.spatial_emb_dim * int(self.if_spatial)
    ...
    def forward(self, batch):
        ...
        time_series = (
            time_series.view(batch_size, num_nodes, -1).transpose(1, 2).unsqueeze(-1)
        )
