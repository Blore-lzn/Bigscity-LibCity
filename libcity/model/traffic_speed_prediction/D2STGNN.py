...
class EstimationGate(nn.Module):
    ...
    def __init__(self, node_emb_dim, time_emb_dim, hidden_dim, config=None):
        ...
        self.fully_connected_layer_1 = nn.Linear(
            2 * node_emb_dim + time_dim, hidden_dim
        )
    ...
    def forward(
        ...
        embedding_feat = [
            node_embedding_u.unsqueeze(0)
            .unsqueeze(0)
            .expand(batch_size, self.seq_length, -1, -1),
            node_embedding_d.unsqueeze(0)
            .unsqueeze(0)
            .expand(batch_size, self.seq_length, -1, -1),
        ]
...
class DifBlock(nn.Module):
    def __init__(
        self, hidden_dim, forecast_hidden_dim=256, config=None, data_feature=None
    ):
        """Diffusion block
        ...
        # forecast
        self.forecast_branch = DifForecast(
            hidden_dim, forecast_hidden_dim=forecast_hidden_dim, config=config
        )
        # backcast
...
class InhBlock(nn.Module):
    def __init__(
        self, hidden_dim, num_heads=4, bias=True, forecast_hidden_dim=256, config=None
    ):
        """Inherent block
        ...
        self.transformer_layer = TransformerLayer(
            hidden_dim, num_heads, self.dropout, bias
        )

        # forecast branch
        self.forecast_block = InhForecast(
            hidden_dim, forecast_hidden_dim, config=config
        )
        # backcast branch
...
class DistanceFunction(nn.Module):
    ...
    def forward(self, X, E_d, E_u, T_D, D_W):
        ...
        )  # [batchsize, num_nodes, hidden_dim]
        ...
        if T_D is not None and D_W is not None:
            ...
            )  # hidden state for calculating distance
            X2 = torch.cat(
                [dy_feat, T_D, D_W, emb2], dim=-1
            )  # hidden state for calculating distance
        elif D_W is not None:
            X1 = torch.cat(
                [dy_feat, D_W, emb1], dim=-1
            )  # hidden state for calculating distance
            X2 = torch.cat(
                [dy_feat, D_W, emb2], dim=-1
            )  # hidden state for calculating distance
        elif T_D is not None:
            X1 = torch.cat(
                [dy_feat, T_D, emb1], dim=-1
            )  # hidden state for calculating distance
            X2 = torch.cat(
                [dy_feat, T_D, emb2], dim=-1
            )  # hidden state for calculating distance
        else:
            X1 = torch.cat(
                [dy_feat, emb1], dim=-1
...
class D2STGNN(AbstractTrafficStateModel):
    ...
    def _graph_constructor(
        ...
        if self.sta_graph:
            static_graph = [
                F.softmax(F.relu(torch.mm(node_embedding_u, node_embedding_d.T)), dim=1)
            ]
        else:
            static_graph = []
        if self.dy_graph:
            dynamic_graph = self.dynamic_graph_constructor(
                history_data,
                node_embedding_d,
                node_embedding_u,
                time_in_day_feat,
    ...
    def _prepare_inputs(self, history_data):
        ...
        if self.add_time_in_day and self.add_day_in_week:
            ...
            day_in_week_feat = self.D_i_W_emb[
                (history_data[:, :, :, num_feat + 1 : num_feat + 8].argmax(dim=3)).type(
                    torch.LongTensor
                )
            ]  # [B, L, N, d]
        elif self.add_time_in_day:
            ...
            day_in_week_feat = self.D_i_W_emb[
                (history_data[:, :, :, num_feat : num_feat + 7].argmax(dim=3)).type(
                    torch.LongTensor
                )
            ]  # [B, L, N, d]
    ...
    def forward(self, batch):
        ...
        forecast = forecast.view(
            ...
            self.gap,
            self.output_dim,
        )  # (B, T / gap, N, gap, output_dim)
        forecast = (
            forecast.transpose(1, 2)
            .contiguous()
            .view(forecast.shape[0], forecast.shape[2], -1, self.output_dim)
        )  # (B, N, T, output_dim)
    ...
    def calculate_loss(self, batch, batches_seen=None):
        ...
        if self.training:
            if (
                batches_seen % (self.cl_decay_epochs * self.num_batches) == 0
                and self.task_level < self.output_window
            ):
                self.task_level += 1
                self._logger.info(
