...
class DKFN(AbstractTrafficStateModel):
    ...
    def __init__(self, config, data_feature):
        ...
        for i in range(self.K):
            ...
            if config.get("Clamp_A", True):
                ...
                A_temp = torch.clamp(A_temp, max=1.0)
        ...
        self.Neighbor_weight = Parameter(
            torch.FloatTensor(self.out_features).to(self.device)
        )
    ...
    def forward(self, batch):
        ...
        Hidden_State, Cell_State, rHidden_State, rCell_State = self.initHidden(
            batch_size
        )
        ...
        return (
            pred.reshape(batch_size, self.output_dim, self.num_nodes)
            .transpose(1, 2)
            .unsqueeze(1)
        )
    ...
    def initHidden(self, batch_size):
        ...
        rHidden_State = Variable(
            torch.zeros(batch_size, self.hidden_size).to(self.device)
        )
        rCell_State = Variable(
            torch.zeros(batch_size, self.hidden_size).to(self.device)
        )
