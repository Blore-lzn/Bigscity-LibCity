...
class GeomGCNSingleChannel(nn.Module):
    def __init__(
        ...
        for i in range(self.num_divisions):
            self.linear_for_each_division.append(
                nn.Linear(in_feats, out_feats, bias=False).to(self.device)
            )

        for i in range(self.num_divisions):
            nn.init.xavier_uniform_(
                self.linear_for_each_division[i].weight.to(self.device)
            )
    ...
    def forward(self, feature):
        in_feats_dropout = self.in_feats_dropout(feature).to(
            self.device
        )  # 使输入挂上dropout
        self.g.ndata["h"] = in_feats_dropout.to(
            self.device
        )  # 使数据挂上dropout；ndata代表特征；加入关于h的索引；
        ...
        for i in range(self.num_divisions):
            ...
            subgraph.update_all(
                message_func=fn.copy_u(u=f"Wh_{i}", out=f"m_{i}"),
        ...
        if self.merge == "cat":
            ...
            h_new = torch.mean(
                torch.stack(results_from_subgraph_list, dim=-1), dim=-1
            ).to(self.device)
...
class GeomGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        (
            ...
            num_heads_layer_two,
            dropout_rate,
            layer_one_ggcn_merge,
            layer_one_channel_merge,
            layer_two_ggcn_merge,
            layer_two_channel_merge,
        ) = self.get_input(config, data_feature)

        self.geomgcn1 = GeomGCNNet(
            g,
            num_input_features,
            num_hidden,
            num_divisions,
        ...
        self.geomgcn2 = GeomGCNNet(
            g,
            num_hidden
            * layer_one_ggcn_merge_multiplier
            * layer_one_channel_merge_multiplier,
            num_output_classes,
            num_divisions,
            lambda x: x,
            num_heads_layer_two,
    ...
    def get_input(self, config, data_feature):
        ...
        for u, v, feature in G.edges(data="subgraph_idx"):
            if (feature is not None) and g.has_edge_between(u, v):
                g.edges[g.edge_id(u, v)].data["subgraph_idx"] = torch.tensor(
                    [feature]
                ).to(self.device)
