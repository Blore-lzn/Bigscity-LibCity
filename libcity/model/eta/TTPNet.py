...
class ShortSpeed(nn.Module):
    def __init__(self, data_feature):
        ...
        self.short_rnn = nn.RNN(
            input_size=4, hidden_size=16, num_layers=1, batch_first=True
        )
        self.data_feature = data_feature
    ...
    def forward(self, batch):
        ...
        grid_len_short = grid_len.expand(
            speeds_forward.size()[:2] + (grid_len.size()[-1],)
        )  # (B * L, 4, 1)
        ...
        speeds_forward = normalize(
            speeds_forward,
            self.data_feature["speeds_mean"],
            self.data_feature["speeds_std"],
        )
        speeds_adjacent1 = normalize(
            speeds_adjacent1,
            self.data_feature["speeds_relevant1_mean"],
            self.data_feature["speeds_relevant1_std"],
        )
        speeds_adjacent2 = normalize(
            speeds_adjacent2,
            self.data_feature["speeds_relevant2_mean"],
            self.data_feature["speeds_relevant2_std"],
        )
        grid_len_short = normalize(
            grid_len_short,
            self.data_feature["grid_len_mean"],
            self.data_feature["grid_len_std"],
        )
        times_forward = normalize(
            times_forward,
            self.data_feature["time_gap_mean"],
            self.data_feature["time_gap_std"],
        )
        times_adjacent1 = normalize(
            times_adjacent1,
            self.data_feature["time_gap_mean"],
            self.data_feature["time_gap_std"],
        )
        times_adjacent2 = normalize(
            times_adjacent2,
            self.data_feature["time_gap_mean"],
            self.data_feature["time_gap_std"],
        )
        ...
        inputs_0 = torch.cat(
            [speeds_forward, grid_len_short, times_forward], dim=2
        )  # (B * L, 4, 3)
        inputs_1 = torch.cat(
            [speeds_adjacent1, grid_len_short, times_adjacent1], dim=2
        )  # (B * L, 4, 3)
        inputs_2 = torch.cat(
            [speeds_adjacent2, grid_len_short, times_adjacent2], dim=2
        )  # (B * L, 4, 3)
        ...
        V_short = torch.cat(
            [outputs_0[:, :, -1], outputs_1[:, :, -1], outputs_2[:, :, -1]], dim=2
        )
...
class LongSpeed(nn.Module):
    ...
    def forward(self, batch):
        ...
        grid_len_long = grid_len.expand(
            speeds_history.size()[:2] + (grid_len.size()[-1],)
        )
        ...
        speeds_history = normalize(
            speeds_history,
            self.data_feature["speeds_long_mean"],
            self.data_feature["speeds_long_std"],
        )
        grid_len_long = normalize(
            grid_len_long,
            self.data_feature["grid_len_mean"],
            self.data_feature["grid_len_std"],
        )
        times_history = normalize(
            times_history,
            self.data_feature["time_gap_mean"],
            self.data_feature["time_gap_std"],
        )
...
class SpeedLSTM(nn.Module):
    ...
    def forward(self, batch):
        ...
        packed_inputs = nn.utils.rnn.pack_padded_sequence(
            whole_t, lens, batch_first=True
        )
        packed_hiddens, (_, _) = self.speed_lstm(packed_inputs)
        speeds_hiddens, _ = nn.utils.rnn.pad_packed_sequence(
...
class RoadLSTM(nn.Module):
    ...
    def forward(self, batch):
        ...
        Roads_hiddens, _ = nn.utils.rnn.pad_packed_sequence(
            packed_hiddens, batch_first=True
        )
...
class PredictionBiLSTM(nn.Module):
    def __init__(self, embed_dims, data_feature):
        ...
        self.lnhiddens = nn.LayerNorm(
            self.attr_net.out_size() + 64, elementwise_affine=True
        )
...
class TTPNet(AbstractTrafficStateModel):
    ...
    def calculate_loss(self, batch):
        if self.training:
            ...
            loss_f = torch.bmm(M_f, torch.pow((T_f_hat - T_f) / T_f, 2)) / torch.bmm(
                M_f, M_f.permute(0, 2, 1)
            )
