    ...
    def __init__(self, c_in, c_out):
        ...
        if c_in > c_out:
            self.conv1x1 = nn.Conv2d(
                c_in, c_out, 1, stride=1, padding=0
            )  # filter=(1,1)
...
class ConvST(nn.Module):
    def __init__(self, supports, kt, ks, dim_in, dim_out, device):
        ...
        self.weights = nn.Parameter(
            torch.FloatTensor(2 * self.dim_out, self.ks * self.kt * self.dim_in).to(
                self.device
            )
        )
    ...
    def forward(self, x):
        ...
        padding = torch.zeros(batch_size, self.dim_in, self.kt - 1, num_nodes).to(
            self.device
        )
        ...
        x = torch.stack(
            [x[:, :, i : i + self.kt, :] for i in range(0, len_time)], dim=2
        )
        # inputs.shape = [B, dim_in, len_time, kt, N]
        x = torch.reshape(x, (-1, num_nodes, self.kt * self.dim_in))
        # inputs.shape = [B*len_time, N, kt*dim_in]
        conv_out = self.graph_conv(
            x, self.supports, self.kt * self.dim_in, 2 * self.dim_out
        )
        ...
        out = (conv_out[:, : self.dim_out, :, :] + res_input) * torch.sigmoid(
            conv_out[:, self.dim_out :, :, :]
        )
    ...
    def graph_conv(self, inputs, supports, dim_in, dim_out):
        ...
        x_new = torch.reshape(
            torch.matmul(x_new, supports), (-1, dim_in, self.ks, num_nodes)
        )
        ...
        outputs = F.linear(
            x_new, self.weights, self.biases
        )  # [batch*num_nodes, dim_out]
        outputs = torch.reshape(
            outputs, [-1, num_nodes, dim_out]
        )  # [batch, num_nodes, dim_out]
        return outputs
...
class AttentionT(nn.Module):
    def __init__(self, device, len_time, num_nodes, d_out, ext_dim):
        ...
        self.weight1 = nn.Parameter(
            torch.FloatTensor(self.len_time, self.num_nodes * self.d_out, 1).to(
                self.device
            )
        )
        self.weight2 = nn.Parameter(
            torch.FloatTensor(self.ext_dim, self.len_time).to(self.device)
        )
    ...
    def forward(self, query, x):
        ...
        score = (
            torch.reshape(torch.matmul(x, self.weight1), (-1, self.len_time))
            + self.bias
        )
        ...
        x = (
            x.permute(0, 2, 1)
            .reshape((-1, 1, self.num_nodes, self.d_out))
            .permute(0, 3, 1, 2)
        )
...
class AttentionC(nn.Module):
    def __init__(self, device, num_nodes, d_out, ext_dim):
        ...
        self.ext_dim = ext_dim
        self.weight1 = nn.Parameter(
            torch.FloatTensor(self.d_out, self.num_nodes, 1).to(self.device)
        )
        self.weight2 = nn.Parameter(
            torch.FloatTensor(self.ext_dim, self.d_out).to(self.device)
        )
    ...
    def forward(self, query, x):
        ...
        # x.shape = [d_out, B, N]
        score = (
            torch.reshape(torch.matmul(x, self.weight1), (-1, self.d_out)) + self.bias
        )
        score = score + torch.matmul(query, self.weight2)
        score = torch.softmax(torch.tanh(score), dim=1)
        # score.shape = [B, d_out]
...
class STG2Seq(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.supports = torch.tensor(
            cheb_poly(scaled_laplacian(self.adj_mx), self.ks), dtype=torch.float32
        ).to(self.device)
        ...
        self.attention_t = AttentionT(
            self.device,
            self.input_window + self.window,
            self.num_nodes,
            self.dim_out,
            self.ext_dim,
        )
        self.attention_c_1 = AttentionC(
            self.device, self.num_nodes, self.dim_out, self.ext_dim
        )
        self.attention_c_2 = AttentionC(
            self.device, self.num_nodes, self.dim_out, self.ext_dim
        )
    ...
    def forward(self, batch):
        inputs = batch["X"][
            :, :, :, : self.output_dim
        ].contiguous()  # (B, input_window, N, output_dim)
        ...
        labels = batch["y"][
            :, :, :, : self.output_dim
        ].contiguous()  # (B, output_window, N, output_dim)
        labels = labels.permute(0, 3, 1, 2)  # (B, output_dim, output_window, N)
        labels_ext = batch["y"][
            :, :, 0, self.output_dim :
        ].contiguous()  # (B, output_window, ext_dim)

        long_output = self.long_term_layer(inputs)  # (B, dim_out, input_window, N)
        preds = []

        if self.training:
            label_padding = inputs[
                :, :, -self.window :, :
            ]  # (B, feature_dim, window, N)
            padded_labels = torch.cat(
                (label_padding, labels), dim=2
            )  # (B, feature_dim, window+output_window, N)
            padded_labels = torch.stack(
                [
                    padded_labels[:, :, i : i + self.window, :]
            ...
            for i in range(0, self.output_window):
                ...
                if self.output_dim == 1:
                    ...
                    pred = torch.cat(
                        (
                            self.attention_c_1(ext_input, ls_inputs),
                            self.attention_c_2(ext_input, ls_inputs),
                        ),
                        dim=1,
                    )
                else:
