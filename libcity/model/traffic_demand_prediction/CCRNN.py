...
class CCRNN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if n == k:
            ...
            initemb2 = torch.mm(
                torch.diag(p[: self.n_dim] ** 0.5), n[:, : self.n_dim].t()
            )
    ...
    def forward(self, batch, batches_seen=None):
        ...
        if self.training:
            outputs = self.decoder(
                graph, states, targets, self._compute_sampling_threshold(batches_seen)
            )
        else:
...
class EvolutionCell(nn.Module):
    def __init__(
        ...
        self.graphconv.append(
            GraphConv(input_dim, output_dim, num_nodes, n_supports, max_step)
        )
        for i in range(1, layer):
            self.graphconv.append(
                GraphConv(output_dim, output_dim, num_nodes, n_supports, max_step)
            )
...
class DCGRUCell(nn.Module):
    def __init__(
        ...
        self.ru_gate_g_conv = EvolutionCell(
        ...
        self.candidate_g_conv = EvolutionCell(
            input_size + hidden_size,
            hidden_size,
            num_node,
            n_supports,
    ...
    def forward(
        self, inputs: Tensor, supports: List[Tensor], states
    ) -> Tuple[Tensor, Tensor]:
        """
        ...
        """
        r_u = torch.sigmoid(
            self.ru_gate_g_conv(torch.cat([inputs, states], -1), supports)
        )
        r, u = r_u.split(self.hidden_size, -1)
        c = torch.tanh(
            self.candidate_g_conv(torch.cat([inputs, r * states], -1), supports)
        )
...
class DCRNNEncoder(nn.ModuleList):
    def __init__(
        ...
        self.append(
            DCGRUCell(
                input_size, hidden_size, num_node, n_supports, k_hop, e_layer, n_dim
            )
        )
    ...
    def forward(self, inputs: Tensor, supports: List[Tensor]) -> Tensor:
        ...
        states = list(
            torch.zeros(len(self), b, n, self.hidden_size, device=dv, dtype=dt)
        )
        inputs = list(inputs.transpose(0, 1))

        for i_layer, cell in enumerate(self):
            for i_t in range(t):
                inputs[i_t], states[i_layer] = cell(
                    inputs[i_t], supports, states[i_layer]
                )
...
class DCRNNDecoder(nn.ModuleList):
    def __init__(
        ...
        self.append(
            DCGRUCell(
                output_size, hidden_size, num_node, n_supports, k_hop, e_layer, n_dim
            )
        )
        for _ in range(1, n_layers):
            self.append(
    ...
    def forward(
        ...
        inputs = torch.zeros(
            b, n, self.output_size, device=states.device, dtype=states.dtype
        )
        ...
        for i_t in range(self.n_preds):
            for i_layer in range(n_layers):
                inputs, states[i_layer] = self[i_layer](
                    inputs, supports, states[i_layer]
                )
...
class GraphConv(nn.Module):
    ...
    def forward(self, inputs: Tensor, supports: List[Tensor]):
        ...
        x = x.reshape(-1, n, input_dim, b).transpose(
            0, 3
        )  # (batch_size, num_nodes, input_dim, num_matrices)
