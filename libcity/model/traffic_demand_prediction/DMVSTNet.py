...
class Line(nn.Module):
    def __init__(self, size, embed_dim=128, order=1):
        ...
        if order == 2:
            ...
            # Initialization
            self.contextnodes_embeddings.weight.data = (
                self.contextnodes_embeddings.weight.data.uniform_(-0.5, 0.5) / embed_dim
            )

        # Initialization
        self.nodes_embeddings.weight.data = (
            self.nodes_embeddings.weight.data.uniform_(-0.5, 0.5) / embed_dim
        )
    ...
    def forward(self, v_i, v_j, negsamples, device):
        ...
        mulnegativebatch = torch.mul(
            v_i.view(len(v_i), 1, self.embed_dim), negativenodes
        )
        negativebatch = torch.sum(
            F.logsigmoid(torch.sum(mulnegativebatch, dim=2)), dim=1
        )
...
def makeData(samplededges, negsamplesize, weights, nodedegrees, nodesaliassampler):
    for e in samplededges:
        ...
        negnodes = []
        for negsample in negSampleBatch(
            sourcenode,
            targetnode,
            negsamplesize,
            weights,
...
class SemanticView(nn.Module):
    def __init__(self, config, data_feature):
        ...
        [edgedistdict, nodedistdict, weights, nodedegrees] = data_feature.get(
            "dtw_graph"
        )
        ...
        self.embedding = (
            line.get_embeddings()
            .reshape((self.len_row, self.len_column, -1))
            .to(self.device)
        )
...
class DMVSTNet(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        # 三层Local CNN
        self.local_conv1 = SpatialViewConv(
            inp_channel=self.feature_dim,
            oup_channel=self.cnn_hidden_dim_first,
            kernel_size=3,
            stride=1,
            padding=1,
        )
        self.local_conv2 = SpatialViewConv(
            inp_channel=self.cnn_hidden_dim_first,
            oup_channel=self.cnn_hidden_dim_first,
            kernel_size=3,
            stride=1,
            padding=1,
        )
        self.local_conv3 = SpatialViewConv(
            inp_channel=self.cnn_hidden_dim_first,
            oup_channel=self.cnn_hidden_dim_first,
            kernel_size=3,
            stride=1,
        ...
        # 全连接降维
        self.fc1 = nn.Linear(
            in_features=self.cnn_hidden_dim_first
        ...
        # 输出层
        self.fc2 = nn.Linear(
            in_features=self.lstm_oup_dim + self.semantic_dim,
    ...
    def forward(self, batch):
        ...
        x = batch["X"].permute(
            0, 1, 4, 2, 3
        )  # (B, input_window, feature_dim, len_row, len_col)
        ...
        # 构造输出
        oup = torch.zeros(
            (batch_size, 1, self.len_row, self.len_column, self.output_dim)
        ).to(self.device)
        # 对每个grid进行预测
            ...
            for j in range(self.padding_size, self.len_column - self.padding_size):
                spatial_res = self.spatial_forward(
                    x_padding[
                        :,
                        :,
                        i - self.padding_size : i + self.padding_size + 1,
                        j - self.padding_size : j + self.padding_size + 1,
                    ]
                )
                # print('spatial_res', spatial_res.shape)  # (B*T, fc_oup_dim)
                seq_res = spatial_res.reshape(
                    (batch_size, self.input_window, self.fc_oup_dim)
                ).permute(1, 0, 2)
    ...
    def predict(self, batch):
        ...
        for i in range(self.output_window):
            ...
            y_ = self.forward(
                batch_tmp
            )  # (batch_size, 1, len_row, len_column, output_dim)
        ...
        y_preds = torch.cat(
            y_preds, dim=1
        )  # (batch_size, output_length, len_row, len_column, output_dim)
        return y_preds
