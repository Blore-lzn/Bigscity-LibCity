...
class Attn(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        self.sqrt_rec_size = 1.0 / sqrt(hidden_size)
        # context vector
...
class ATSTLSTM(AbstractModel):
    ...
    def __init__(self, config, data_feature):
        ...
        self.user_embedding = nn.Embedding(
            num_embeddings=self.uid_size, embedding_dim=self.hidden_size
        )
        # Wv
        self.wv = nn.Linear(
    ...
    def forward(self, batch):
        ...
        # embedding
        total_loc_emb = self.loc_embedding(
            total_loc
        )  # batch_size * total_len * hidden_size
        total_emb = self.wv(total_loc_emb) + self.wl(total_dis) + self.wt(total_tim)
        # split emb
        current_emb, rest_emb = torch.split(
            total_emb, [padded_seq_len, total_emb.shape[1] - padded_seq_len], dim=1
        )
        # lstm
        pack_current_emb = pack_padded_sequence(
            current_emb, lengths=origin_len, enforce_sorted=False, batch_first=True
        )
        ...
        output = torch.bmm(rest_emb, first_part).squeeze(
            2
        )  # batch_size * (neg_samples+1)
