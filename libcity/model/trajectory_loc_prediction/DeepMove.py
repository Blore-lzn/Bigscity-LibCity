...
class DeepMove(AbstractModel):
    ...
    def __init__(self, config, data_feature):
        ...
        self.emb_loc = nn.Embedding(
            self.loc_size, self.loc_emb_size, padding_idx=data_feature["loc_pad"]
        )
        self.emb_tim = nn.Embedding(
            self.tim_size, self.tim_emb_size, padding_idx=data_feature["tim_pad"]
        )
    ...
    def forward(self, batch):
        ...
        final_out_index = final_out_index.repeat(1, 1, 2 * self.hidden_size).to(
            self.device
        )
