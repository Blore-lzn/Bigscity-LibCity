...
class EmbeddingMatrix(nn.Module):  # text_embdeding
    def __init__(self, input_size, output_size, word_vec):
        ...
        self.layer = nn.Linear(
            in_features=self.input_size, out_features=self.output_size, bias=False
        )
...
class SERM(AbstractModel):
    def __init__(self, config, data_feature):
        ...
        self.emb_loc = nn.Embedding(
            num_embeddings=self.loc_size,
            embedding_dim=self.loc_emb_size,
            padding_idx=data_feature["loc_pad"],
        )
        self.emb_tim = nn.Embedding(
            num_embeddings=self.tim_size,
            embedding_dim=self.tim_emb_size,
        ...
        self.emb_text = EmbeddingMatrix(
            self.text_size, self.text_emb_size, data_feature["word_vec"]
        )

        # lstm layer
        self.lstm = nn.LSTM(
            input_size=self.loc_emb_size + self.tim_emb_size + self.text_emb_size,
    ...
    def forward(self, batch):
        ...
        for word_index in text:
            ...
            for words in word_index:
                if len(words) == 0:
                    ...
                    one_hot_text_a_slice.append(
                        np.sum(self.word_one_hot_matrix[words], axis=0) / len(words)
                    )
            # pad
            one_hot_text_a_slice += [text_pad] * (max_len - len(one_hot_text_a_slice))
            one_hot_text.append(
                np.array(one_hot_text_a_slice)
            )  # batch_size * seq_len * text_size
        ...
        for i in range(lstm_out.shape[0]):
            if i == 0:
                out = lstm_out[0][seq_len[i] - 1].reshape(
                    1, -1
