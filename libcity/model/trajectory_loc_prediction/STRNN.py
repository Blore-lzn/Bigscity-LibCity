...
class STRNN(AbstractModel):
    def __init__(self, config, data_feature):
        ...
        self.location_weight = nn.Embedding(
            self.loc_size, self.hidden_size
        )  # 还是按编号来的，但是需要经纬度额外信息
    ...
    def forward(self, td_upper, td_lower, ld_upper, ld_lower, current_loc, loc_len):
        ...
        for i in range(batch_size):
            ...
            # hidden_size x 1
            hx = (loc_vec + usr_vec).reshape(1, self.hidden_size)
            output.append(hx)
    ...
    def calculate_loss(self, batch):
        ...
        td_upper = (
            torch.LongTensor([self.up_time] * batch_size).to(self.device).unsqueeze(1)
        )
        td_upper = td_upper - td
        td_lower = td  # 因为 lower 是 0
        ld_upper = (
            torch.LongTensor([self.up_loc] * batch_size).to(self.device).unsqueeze(1)
        )
        ld_upper = ld_upper - ld
        ld_lower = ld  # 因为下界是 0
        # batch_size * hidden_size
        h_tq = self.forward(
            td_upper, td_lower, ld_upper, ld_lower, batch["current_loc"], loc_len
        )
        dst = batch["target"]
