...
class Embed(nn.Module):
    ...
    def forward(self, traj_loc, mat2, vec, traj_len):
        ...
        for i in range(mask.shape[0]):  # N
            ...
            delta_s[i, : traj_len[i]] = torch.index_select(
                mat2, 0, (traj_loc[i] - 1)[: traj_len[i]]
            )
...
class STAN(AbstractModel):
    ...
    def predict(self, batch):
        ...
        for i in range(traj_spatial_mat.shape[0]):
            traj_loc_dis = torch.index_select(
                self.mat2s, 0, (traj_loc[i] - 1)[: traj_len[i]]
            )
            traj_spatial_mat[i, : traj_len[i], : traj_len[i]] = torch.index_select(
                traj_loc_dis, 1, (traj_loc[i] - 1)[: traj_len[i]]
            )
        mat1 = torch.cat(
            (traj_spatial_mat.unsqueeze(3), traj_temporal_mat.unsqueeze(3)), 3
        )
        return self.forward(
            batch["traj"], mat1, batch["candiate_temporal_vec"], batch["traj_len"]
        )
    ...
    def _sampling_prob(self, score, label):
        ...
        init_prob = torch.zeros(
            size=(num_label, self.num_neg + len(label))
        )  # (N, num_neg+num_label)
        random_ig = random.sample(
            range(1, l_m + 1), self.num_neg
        )  # (num_neg) from (1 -- l_max)
        ...
        return torch.FloatTensor(init_prob), torch.LongTensor(
            init_label
        )  # (N, num_neg+num_label), (N)
