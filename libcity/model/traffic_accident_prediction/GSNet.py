...
class STGeoModule(nn.Module):
    def __init__(
        ...
        self.grid_conv = nn.Sequential(
            nn.Conv2d(
                in_channels=grid_in_channel, out_channels=64, kernel_size=3, padding=1
            ),
            nn.ReLU(),
            nn.Conv2d(
                in_channels=64, out_channels=grid_in_channel, kernel_size=3, padding=1
            ),
            nn.ReLU(),
        )

        self.grid_gru = nn.GRU(
            grid_in_channel, gru_hidden_size, num_of_gru_layers, batch_first=True
        )

        self.grid_att_fc1 = nn.Linear(in_features=gru_hidden_size, out_features=1)
        self.grid_att_fc2 = nn.Linear(
            in_features=num_of_target_time_feature, out_features=input_window
        )
    ...
    def forward(self, grid_input, target_time_feature):
        ...
        conv_output = (
            conv_output.view(batch_size, -1, D, W, H)
            .permute(0, 3, 4, 1, 2)
            .contiguous()
            .view(-1, T, D)
        )
...
class STSemModule(nn.Module):
    def __init__(
        ...
        for idx, num_of_filter in enumerate(nums_of_graph_filters):
            if idx == 0:
                ...
                self.road_gcn.append(
                    GCNLayer(nums_of_graph_filters[idx - 1], num_of_filter)
                )
        ...
        self.graph_gru = nn.GRU(
            num_of_filter, gru_hidden_size, num_of_gru_layers, batch_first=True
        )

        self.graph_att_fc1 = nn.Linear(in_features=gru_hidden_size, out_features=1)
        self.graph_att_fc2 = nn.Linear(
            in_features=num_of_target_time_feature, out_features=input_window
        )
    ...
    def forward(
        ...
        grid_node_map_tmp = grid_node_map.to(graph_feature.device).repeat(
            batch_size, 1, 1
        )
        graph_output = (
            torch.bmm(grid_node_map_tmp, graph_output)
            .permute(0, 2, 1)
...
class _GSNet(nn.Module):
    def __init__(
        ...
        self.st_geo_module = STGeoModule(
            grid_in_channel,
            num_of_gru_layers,
            input_window,
            gru_hidden_size,
        ...
        self.st_sem_module = STSemModule(
            num_of_graph_feature,
            nums_of_graph_filters,
            input_window,
            num_of_gru_layers,
    ...
    def forward(
        ...
        graph_output = self.st_sem_module(
            graph_feature,
            road_adj,
            risk_adj,
            poi_adj,
        ...
        final_output = self.output_layer(fusion_output).view(
            batch_size, -1, self.north_south_map, self.west_east_map
        )
        return final_output
...
class GSNet(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.num_of_target_time_feature = data_feature.get(
            "num_of_target_time_feature", 0
        )
        ...
        self.risk_mask = data_feature.get(
            "risk_mask", torch.Tensor(size=(self.north_south_map, self.west_east_map))
        )
        self.road_adj = data_feature.get(
            "road_adj", torch.Tensor(size=(self.north_south_map, self.west_east_map))
        )
        self.risk_adj = data_feature.get(
            "risk_adj", torch.Tensor(size=(self.north_south_map, self.west_east_map))
        )
        self.poi_adj = data_feature.get(
            "poi_adj", torch.Tensor(size=(self.north_south_map, self.west_east_map))
        )
        self.grid_node_map = data_feature.get(
            "grid_node_map",
            torch.Tensor(size=(self.north_south_map, self.west_east_map)),
        )

        self.dtype = config.get("dtype", torch.float32)
        self.risk_mask = torch.from_numpy(self.risk_mask).to(
            device=self.device, dtype=self.dtype
        )
        self.road_adj = torch.from_numpy(self.road_adj).to(
            device=self.device, dtype=self.dtype
        )
        self.risk_adj = torch.from_numpy(self.risk_adj).to(
            device=self.device, dtype=self.dtype
        )
        if self.poi_adj is not None:
            self.poi_adj = torch.from_numpy(self.poi_adj).to(
                device=self.device, dtype=self.dtype
            )
        self.grid_node_map = torch.from_numpy(self.grid_node_map).to(
            device=self.device, dtype=self.dtype
        )
        ...
        self.gsnet = _GSNet(
            ...
            num_of_gru_layers=self.num_of_gru_layers,
            gru_hidden_size=self.gru_hidden_size,
            nums_of_graph_filters=self.nums_of_graph_filters,
            num_of_graph_feature=self.num_of_graph_feature,
    ...
    def forward(self, batch):
        ...
        graph_input = (
            grid_input[:, :, self.graph_input_indices, :, :]
            .reshape(
                batch_size,
                self.input_window,
    ...
    def calculate_loss(self, batch):
        ...
        for i in range(len(self.risk_thresholds) + 1):
            if i == 0:
                ...
                weight[
                    (y_true > self.risk_thresholds[i - 1])
                    & (y_true <= self.risk_thresholds[i])
                ] = self.risk_weights[i]
