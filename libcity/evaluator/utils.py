        ...
        for metric in metrics:
            if mode.lower() == "single":
                if metric == "masked_MAE":
                    line[metric] = masked_mae_torch(
                        y_pred[:, i - 1], y_true[:, i - 1], 0
                    ).item()
                elif metric == "masked_MSE":
                    line[metric] = masked_mse_torch(
                        y_pred[:, i - 1], y_true[:, i - 1], 0
                    ).item()
                elif metric == "masked_RMSE":
                    line[metric] = masked_rmse_torch(
                        y_pred[:, i - 1], y_true[:, i - 1], 0
                    ).item()
                elif metric == "masked_MAPE":
                    line[metric] = masked_mape_torch(
                        y_pred[:, i - 1], y_true[:, i - 1], 0
                    ).item()
                elif metric == "MAE":
                    line[metric] = masked_mae_torch(
                    ...
                    line[metric] = r2_score_torch(
                        y_pred[:, i - 1], y_true[:, i - 1]
                    ).item()
                elif metric == "EVAR":
                    line[metric] = explained_variance_score_torch(
                        y_pred[:, i - 1], y_true[:, i - 1]
                    ).item()
                else:
                    raise ValueError(
                        "Error parameter mode={}, please set `single` or `average`.".format(
                            mode
                        )
                    )
