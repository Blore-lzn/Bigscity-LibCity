    ...
    def _check_config(self):
        ...
        for metric in self.metrics:
            if metric not in self.allowed_metrics:
                raise ValueError(
                    "the metric {} is not allowed in TrafficAccidentEvaluator".format(
                        str(metric)
                    )
                )
            ...
            for i in range(1, self.len_timeslots + 1):
                for metric in self.metrics:
                    if metric == "masked_MAE":
                        ...
                        self.intermediate_result[metric + "@" + str(i)].append(
                            loss.masked_rmse_torch(
                                y_pred[:, :i], y_true[:, :i], 0
                            ).item()
                        )
                    elif metric == "masked_MAPE":
                        self.intermediate_result[metric + "@" + str(i)].append(
                            loss.masked_mape_torch(
                                y_pred[:, :i], y_true[:, :i], 0
                            ).item()
                        )
                    elif metric == "MAE":
            ...
            for i in range(1, self.len_timeslots + 1):
                for metric in self.metrics:
                    if metric == "masked_MAE":
                        ...
                        self.intermediate_result[metric + "@" + str(i)].append(
                            eval_funcs.Precision_torch(
                                y_pred[:, i - 1], y_true[:, i - 1], self.topk
                            )
                        )
                    elif metric == "Recall":
                        self.intermediate_result[metric + "@" + str(i)].append(
                            eval_funcs.Recall_torch(
                                y_pred[:, i - 1], y_true[:, i - 1], self.topk
                            )
                        )
                    elif metric == "F1-Score":
                        self.intermediate_result[metric + "@" + str(i)].append(
                            eval_funcs.F1_Score_torch(
                                y_pred[:, i - 1], y_true[:, i - 1], self.topk
                            )
                        )
                    elif metric == "MAP":
                        self.intermediate_result[metric + "@" + str(i)].append(
                            eval_funcs.MAP_torch(
                                y_pred[:, i - 1], y_true[:, i - 1], self.topk
                            )
                        )
                    elif metric == "PCC":
                        self.intermediate_result[metric + "@" + str(i)].append(
                            eval_funcs.PCC_torch(
                                y_pred[:, i - 1], y_true[:, i - 1], self.topk
                            )
                        )
        else:
