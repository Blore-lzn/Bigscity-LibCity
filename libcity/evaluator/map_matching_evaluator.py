    ...
    def save_result(self, save_path, filename=None):
        ...
        for usr_id, usr_value in self.merged_result.items():
            for traj_id, merged_result in usr_value.items():
                ...
                lon_last = None
                for rel_id in merged_result:
                    lat_origin = self.rd_nwk.nodes[self.rel_info[rel_id]["point1"]][
                        "lat"
                    ]
                    lon_origin = self.rd_nwk.nodes[self.rel_info[rel_id]["point1"]][
                        "lon"
                    ]
                    lat_destination = self.rd_nwk.nodes[
                        self.rel_info[rel_id]["point2"]
                    ]["lat"]
                    lon_destination = self.rd_nwk.nodes[
                        self.rel_info[rel_id]["point2"]
                    ]["lon"]
        ...
        json.dump(
            geojson_obj,
            open(save_path + "/" + filename + "_result.json", "w", encoding="utf-8"),
            ensure_ascii=False,
            indent=4,
        )
        ...
        if self.route is not None:
            ...
            if "json" in self.save_modes:
                self._logger.info(
                    "Evaluate result is " + json.dumps(self.evaluate_result)
                )
                with open(
            ...
            if "csv" in self.save_modes:
                ...
                df.to_csv(
                    os.path.join(save_path, "{}.csv".format(filename)), index=False
                )
                self._logger.info(
                    "Evaluate result is saved at "
    ...
    def find_lcs(self):
        ...
        for usr_id, usr_value in self.route.items():
            for traj_id, result in usr_value.items():
                ...
                res = [
                    [0 for _ in range(len1 + 1)] for _ in range(len2 + 1)
                                ...
                                try:
                                    ...
                                    while j < len(path) - 1:
                                        ...
                                        for rel_id in self.rel_info.keys():
                                            if (
                                                self.rel_info[rel_id]["point1"]
                                                == point1
                                                and self.rel_info[rel_id]["point2"]
                                                == point2
                                            ):
