...
class TrajLocPredEvaluator(AbstractEvaluator):
    ...
    def collect(self, batch):
        ...
        if type(self.topk) == type(0):
            ...
            for idx in range(len(self.topk)):
                hit, rank, dcg = top_k(
                    batch["loc_pred"], batch["loc_true"], self.topk[idx]
                )
    ...
    def evaluate(self):
        if type(self.topk) == type(0):
            ...
            recall = self.intermediate_result["hit"] / self.intermediate_result["total"]
            ...
            if "F1" in self.metrics:
                ...
                if precision + recall == 0:
                    ...
                    self.result[f1_key] = (2 * precision * recall) / (
                        precision + recall
                    )
            if "MRR" in self.metrics:
                mrr_key = "MRR@{}".format(self.topk)
                self.result[mrr_key] = (
                    self.intermediate_result["rank"] / self.intermediate_result["total"]
                )
            ...
            if "NDCG" in self.metrics:
                ...
                self.result[ndcg_key] = (
                    self.intermediate_result["dcg"] / self.intermediate_result["total"]
                )
        elif type(self.topk) == type([]):
            for k in self.topk:
                ...
                recall = (
                    self.intermediate_result["hit" + str(k)]
                ...
                if "MRR" in self.metrics:
                    ...
                    self.result[mrr_key] = (
                        self.intermediate_result["rank" + str(k)]
                        / self.intermediate_result["total"]
                    )
                if "MAP" in self.metrics:
                ...
                if "NDCG" in self.metrics:
                    ...
                    self.result[ndcg_key] = (
                        self.intermediate_result["dcg" + str(k)]
                        / self.intermediate_result["total"]
                    )
    ...
    def save_result(self, save_path, filename=None):
        ...
        self._logger.info(
            "evaluate result is {}".format(json.dumps(self.result, indent=1))
        )
