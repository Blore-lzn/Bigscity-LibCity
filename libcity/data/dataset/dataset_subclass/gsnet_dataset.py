...
class GSNetDataset(TrafficStateCPTDataset):
    def __init__(self, config):
        ...
        self.num_of_target_time_feature = self.config.get(
        ...
        self.data_col_grid_node_map = self.config.get(
            "data_col_grid_node_map", "grid_node_map"
        )
    ...
    def _load_risk_mask(self, filename):
        ...
        self.risk_mask = np.array(risk_mask_values, dtype=np.float32).reshape(
            len_column, len_row
        )
        self.grid_node_map = np.array(grid_node_map_values, dtype=np.float32).reshape(
            len_column * len_row, num_graph_nodes
        )
    ...
    def get_data_feature(self):
        ...
        if len(d["risk_thresholds"]) != len(d["risk_weights"]) - 1:
            raise ValueError(
                "Mask loss risk thresholds must be one element shorter than risk weights"
            )
