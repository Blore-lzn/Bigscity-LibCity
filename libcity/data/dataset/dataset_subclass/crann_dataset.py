...
class CRANNDataset(TrafficStatePointDataset):
    ...
    def _generate_data(self):
        ...
        indexes2 = (
            indexes1
            + (self.n_timesteps + self.output_window)
            - (self.input_window + self.output_window)
        )
        for index1, index2 in zip(indexes1, indexes2):
            data_time.append(
                ext_data[index1 : index1 + (self.n_timesteps + self.output_window), 0:1]
            )
            data_space.append(
                df[
                    index2 : index2 + (self.input_window + self.output_window),
                    0 : self.dim_x * self.dim_y,
                ]
            )
            data_ext.append(
                ext_data[
                    (index2 + self.input_window) : index2
                    + (self.input_window + self.output_window),
                    1:,
                ]
            )

        data_time = np.array(data_time).reshape(
            (-1, (self.n_timesteps + self.output_window), 1)
        )
        data_space = np.array(data_space).reshape(
            (-1, (self.input_window + self.output_window), self.dim_x, self.dim_y)
        )
        data_ext = np.array(data_ext).reshape(
            (-1, self.output_window, ext_data.shape[1] - 1)
        )

        x_time, y_time = (
            data_time[:, : self.n_timesteps, :],
            data_time[:, self.n_timesteps :, :],
        )
        x_space, y_space = (
            data_space[:, : self.input_window, :, :],
            data_space[:, self.input_window :, :, :],
        )
        x_ext = data_ext
    ...
    def _split_train_val_test(self, x_time, x_space, x_ext, y):
        ...
        x_time_val, x_space_val, x_ext_val, y_val = (
            x_time[num_train : num_train + num_val],
            x_space[num_train : num_train + num_val],
            x_ext[num_train : num_train + num_val],
        ...
        self._logger.info(
            "train\t"
            + "x_time: "
            + str(x_time_train.shape)
            + ", x_space: "
            + str(x_space_train.shape)
            + ", x_ext: "
            + str(x_ext_train.shape)
            + ", y: "
            + str(y_train.shape)
        )
        self._logger.info(
            "eval\t"
            + "x_time: "
            + str(x_time_val.shape)
            + ", x_space: "
            + str(x_space_val.shape)
            + ", x_ext: "
            + str(x_ext_val.shape)
            + ", y: "
            + str(y_val.shape)
        ...
        self._logger.info(
            "test\t"
            + "x_time: "
            + str(x_time_test.shape)
            + ", x_space: "
            + str(x_space_test.shape)
            + ", x_ext: "
            + str(x_ext_test.shape)
            + ", y: "
            + str(y_test.shape)
        )
    ...
    def get_data(self):
        ...
        x_time_train[..., : self.output_dim] = self.scaler.transform(
            x_time_train[..., : self.output_dim]
        )
        x_space_train[..., : self.output_dim] = self.scaler.transform(
            x_space_train[..., : self.output_dim]
        )
        x_ext_train[..., : self.output_dim] = self.scaler.transform(
            x_ext_train[..., : self.output_dim]
        )
        y_train[..., : self.output_dim] = self.scaler.transform(
            y_train[..., : self.output_dim]
        )
        x_time_val[..., : self.output_dim] = self.scaler.transform(
            x_time_val[..., : self.output_dim]
        )
        x_space_val[..., : self.output_dim] = self.scaler.transform(
            x_space_val[..., : self.output_dim]
        )
        x_ext_val[..., : self.output_dim] = self.scaler.transform(
            x_ext_val[..., : self.output_dim]
        )
        y_val[..., : self.output_dim] = self.scaler.transform(
            y_val[..., : self.output_dim]
        )
        x_time_test[..., : self.output_dim] = self.scaler.transform(
            x_time_test[..., : self.output_dim]
        )
        x_space_test[..., : self.output_dim] = self.scaler.transform(
            x_space_test[..., : self.output_dim]
        )
        x_ext_test[..., : self.output_dim] = self.scaler.transform(
            x_ext_test[..., : self.output_dim]
        )
