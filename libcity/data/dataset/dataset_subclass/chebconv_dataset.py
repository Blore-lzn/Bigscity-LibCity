...
class ChebConvDataset(AbstractDataset):
    ...
    def _load_rel(self):
        ...
        for i in range(map_info.shape[0]):
            if (
                map_info["origin_id"][i] in self.geo_to_ind
                and map_info["destination_id"][i] in self.geo_to_ind
            ):
                ...
                if (f_id, t_id) not in adj_set:
                    ...
                    adj_data.append(1.0)
                    cnt = cnt + 1
        self.adj_mx = sp.coo_matrix(
            (adj_data, (adj_row, adj_col)), shape=(self.num_nodes, self.num_nodes)
        )
    ...
    def _split_train_val_test(self):
        ...
        np.save(
            self.cache_file_folder + "{}_node_features.npy".format(self.dataset),
            node_features,
        )
    ...
    def _get_scalar(self, scaler_type, data):
        ...
        if scaler_type == "normal":
            ...
            scaler = MinMax01Scaler(maxx=data.max(), minn=data.min())
            self._logger.info(
                "MinMax01Scaler max: " + str(scaler.max) + ", min: " + str(scaler.min)
            )
        elif scaler_type == "minmax11":
            scaler = MinMax11Scaler(maxx=data.max(), minn=data.min())
            self._logger.info(
