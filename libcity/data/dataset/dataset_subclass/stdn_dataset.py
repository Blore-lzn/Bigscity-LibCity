...
class STDNDataset(TrafficStateDataset):
    def __init__(self, config):
        ...
        self.feature_name = {
            "X": "float",
            "y": "float",
            "flatten_att_nbhd_inputs": "float",
            "flatten_att_flow_inputs": "float",
            "att_lstm_inputs": "float",
            "nbhd_inputs": "float",
            "flow_inputs": "float",
            "lstm_inputs": "float",
        }
    ...
    def _load_gridod(self, filename):
        ...
        self.timesolts = list(
            gridodfile["time"][
                : int(gridodfile.shape[0] / len(self.geo_ids) / len(self.geo_ids))
            ]
        )
        self.idx_of_timesolts = dict()
        if not gridodfile["time"].isna().any():  # 时间没有空值
            self.timesolts = list(
                map(lambda x: x.replace("T", " ").replace("Z", ""), self.timesolts)
            )
        ...
        for oi in range(self.len_row):
            for oj in range(self.len_column):
                origin_index = (
                    (oi * self.len_column + oj) * len_time * len(self.geo_ids)
                )  # 每个起点占据len_t*n行
                for di in range(self.len_row):
                    for dj in range(self.len_column):
                        destination_index = (
                            di * self.len_column + dj
                        ) * len_time  # 每个终点占据len_t行
        ...
        self._logger.info(
            "Loaded file " + filename + ".gridod" + ", shape=" + str(data.shape)
        )
        return data
    ...
    def _sample_stdn(self, volume_df, flow_df):
        ...
        time_start = (
            self.hist_feature_daynum + self.att_lstm_num
        ) * self.timeslot_daynum + self.att_lstm_seq_len
                ...
                for y in range(self.len_column):
                    ...
                    for seqn in range(self.input_window):
                        ...
                        cnn_feature = np.zeros(
                            (
                                2 * self.cnn_nbhd_size + 1,
                                2 * self.cnn_nbhd_size + 1,
                                volume_type,
                            )
                        )
                        for cnn_nbhd_x in range(
                            x - self.cnn_nbhd_size, x + self.cnn_nbhd_size + 1
                        ):
                            for cnn_nbhd_y in range(
                                y - self.cnn_nbhd_size, y + self.cnn_nbhd_size + 1
                            ):
                                if not (
                                    0 <= cnn_nbhd_x < self.len_row
                                    and 0 <= cnn_nbhd_y < self.len_column
                                ):
                                    continue
                                cnn_feature[
                                    cnn_nbhd_x - (x - self.cnn_nbhd_size),
                                    cnn_nbhd_y - (y - self.cnn_nbhd_size),
                                    :,
                                ] = volume_df[real_t, cnn_nbhd_x, cnn_nbhd_y, :]
                        cnn_features[seqn].append(cnn_feature)

                        flow_feature_curr_out = flow_df[real_t, x, y, :, :, 0]
                        flow_feature_curr_in = flow_df[real_t, :, :, x, y, 0]
                        flow_feature_last_out_to_curr = flow_df[
                            real_t - 1, x, y, :, :, 1
                        ]
                        flow_feature_curr_in_from_last = flow_df[
                            real_t - 1, :, :, x, y, 1
                        ]
                        ...
                        local_flow_feature = np.zeros(
                            (2 * self.cnn_nbhd_size + 1, 2 * self.cnn_nbhd_size + 1, 4)
                        )
                        for cnn_nbhd_x in range(
                            x - self.cnn_nbhd_size, x + self.cnn_nbhd_size + 1
                        ):
                        ...
                        hist_feature = volume_df[
                            real_t
                            - self.hist_feature_daynum
                            * self.timeslot_daynum : real_t : self.timeslot_daynum,
                            x,
                            y,
                            :,
                        ].flatten()
                        last_feature = volume_df[
                            real_t - self.last_feature_num : real_t, x, y, :
                        ].flatten()
                        nbhd_feature = np.zeros(
                            (
                                2 * self.nbhd_size + 1,
                                2 * self.nbhd_size + 1,
                                volume_type,
                        ...
                        for nbhd_x in range(x - self.nbhd_size, x + self.nbhd_size + 1):
                            for nbhd_y in range(
                                y - self.nbhd_size, y + self.nbhd_size + 1
                            ):
                                if not (
                                    0 <= nbhd_x < self.len_row
                                    and 0 <= nbhd_y < self.len_column
                                ):
                                    continue
                                nbhd_feature[
                                    nbhd_x - (x - self.nbhd_size),
                                    nbhd_y - (y - self.nbhd_size),
                                    :,
                    ...
                    for att_lstm_cnt in range(self.att_lstm_num):
                        ...
                        att_t = (
                            t
                            - (self.att_lstm_num - att_lstm_cnt) * self.timeslot_daynum
                            + (self.att_lstm_seq_len - 1) / 2
                            + 1
                        )
                        att_t = int(att_t)
                        for seqn in range(self.att_lstm_seq_len):
                            ...
                            flow_att_features[att_lstm_cnt][seqn].append(
                                local_flow_feature
                            )
                            ...
                            for nbhd_x in range(
                                x - self.nbhd_size, x + self.nbhd_size + 1
                            ):
                                for nbhd_y in range(
                                    y - self.nbhd_size, y + self.nbhd_size + 1
                                ):
                        ...
                        lstm_att_features[att_lstm_cnt].append(
                            np.array(long_term_lstm_samples)
                        )
        ...
        output_cnn_att_features = np.reshape(
            output_cnn_att_features,
        ...
        output_flow_att_features = np.reshape(
            output_flow_att_features,
        ...
        cnn_features = np.reshape(
            cnn_features, (-1, self.len_row, self.len_column, *cnn_features.shape[1:])
        )
        flow_features = np.stack(flow_features, axis=0)
        flow_features = np.swapaxes(flow_features, 0, 1)
        flow_features = np.reshape(
            flow_features, (-1, self.len_row, self.len_column, *flow_features.shape[1:])
        )
        short_term_lstm_features = np.array(short_term_lstm_features)
        short_term_lstm_features = np.reshape(
            short_term_lstm_features,
    ...
    def _generate_input_data_stdn(self, volume_df, flow_df):
        (
            flatten_att_nbhd_input,
            flatten_att_flow_input,
            att_lstm_input,
            nbhd_input,
            flow_input,
            lstm_input,
        ) = self._sample_stdn(volume_df, flow_df)
    ...
    def _generate_data(self):
        ...
        (
            ...
            flow_inputs,
            lstm_inputs,
        ) = self._generate_input_data_stdn(volume_df, flow_df)
        ...
        self._logger.info(
            ...
            + ", y shape: "
            + str(y.shape)
            + ", flatten_att_nbhd_inputs shape: "
            + str(flatten_att_nbhd_inputs.shape)
            + ", flatten_att_flow_inputs shape: "
            + str(flatten_att_flow_inputs.shape)
            + ", att_lstm_inputs shape: "
            + str(att_lstm_inputs.shape)
            + ", nbhd_inputs shape: "
            + str(nbhd_inputs.shape)
            + ", flow_inputs shape: "
            + str(flow_inputs.shape)
            + ", lstm_inputs shape: "
            + str(lstm_inputs.shape)
        )
        return (
            x,
            y,
    ...
    def _split_train_val_test_stdn(
        self,
        x,
        y,
        flatten_att_nbhd_inputs,
        ...
        self._logger.info(
            ...
            + "att_lstm_inputs: "
            + str(att_lstm_inputs_train.shape)
            + "nbhd_inputs: "
            + str(nbhd_inputs_train.shape)
            + "flow_inputs: "
            + str(flow_inputs_train.shape)
            + "lstm_inputs: "
        ...
        self._logger.info(
            ...
            + "att_lstm_inputs: "
            + str(att_lstm_inputs_val.shape)
            + "nbhd_inputs: "
            + str(nbhd_inputs_val.shape)
            + "flow_inputs: "
            + str(flow_inputs_val.shape)
            + "lstm_inputs: "
        ...
        self._logger.info(
            ...
            + "att_lstm_inputs: "
            + str(att_lstm_inputs_test.shape)
            + "nbhd_inputs: "
            + str(nbhd_inputs_test.shape)
            + "flow_inputs: "
            + str(flow_inputs_test.shape)
            + "lstm_inputs: "
    ...
    def _generate_train_val_test(self):
        ...
        return self._split_train_val_test_stdn(
            x,
            y,
            flatten_att_nbhd_inputs,
            flatten_att_flow_inputs,
    ...
    def _get_scalar_stdn(self, x_train, y_train, flow_inputs_train):
        if self.scaler_type == "normal":
            ...
            self._logger.info(
                "NormalScaler volume max: "
                + str(volume_scaler.max)
                + " flow max: "
                + str(flow_scaler.max)
            )
            ...
            flow_scaler = StandardScaler(
                mean=flow_inputs_train.mean(), std=flow_inputs_train.std()
            )
            self._logger.info(
                "StandardScaler volume mean: "
                + str(volume_scaler.mean)
                + ", volume std: "
                + str(volume_scaler.std)
                + ", flow mean: "
                + str(flow_scaler.mean)
                + ", flow std: "
                + str(flow_scaler.std)
            )
        elif self.scaler_type == "minmax01":
            volume_scaler = MinMax01Scaler(
                maxx=max(x_train.max(), y_train.max()),
                minn=min(x_train.min(), y_train.min()),
            )
            flow_scaler = MinMax01Scaler(
                maxx=flow_inputs_train.max(), minn=flow_inputs_train.min()
            )
            self._logger.info(
                "MinMax01Scaler volume max: "
                + str(volume_scaler.max)
                + ", volume min: "
                + str(volume_scaler.min)
                + ", flow max: "
                + str(flow_scaler.max)
                + ", flow min: "
                + str(flow_scaler.min)
            ...
            volume_scaler = MinMax11Scaler(
                maxx=max(x_train.max(), y_train.max()),
                minn=min(x_train.min(), y_train.min()),
            )
            flow_scaler = MinMax11Scaler(
                maxx=flow_inputs_train.max(), minn=flow_inputs_train.min()
            )
            self._logger.info(
                "MinMax11Scaler volume max: "
                + str(volume_scaler.max)
                + ", volume min: "
                + str(volume_scaler.min)
                + ", flow max: "
