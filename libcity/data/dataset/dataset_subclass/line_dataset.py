...
class LINEDataset(AbstractDataset):
    ...
    def _load_rel(self):
        ...
        if "weight" in map_info.columns:
            self.edges = [
                (self._geo_to_ind[e[0]], self._geo_to_ind[e[1]], e[2])
                for e in map_info[["origin_id", "destination_id", "weight"]].values
            ]
        else:
            self.edges = [
                (self._geo_to_ind[e[0]], self._geo_to_ind[e[1]], 1)
                for e in map_info[["origin_id", "destination_id"]].values
            ]
    ...
    def _generate_data(self):
        ...
        # eval
        I_eval, J_eval, Neg_eval = (
            I[num_train : num_train + num_eval],
            J[num_train : num_train + num_eval],
