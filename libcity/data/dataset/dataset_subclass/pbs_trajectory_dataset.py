...
class PBSTrajectoryDataset(AbstractDataset):
    ...
    def encode_traj(self, data):
        ...
        weight = []
        for key, cnt in tqdm(
            self.counter.most_common(), desc="calculating popularity weight"
        ):
        ...
        for uid in tqdm(data, desc="encoding trajectory"):
            ...
            for i in range(len(data[uid])):
                neg = np.random.choice(
                    popularity, size=self.neg_samples, replace=False, p=weight
                )
                total_negative_samples.append(neg)
            encoded_data[uid] = self.encoder.encode(
                int(uid), data[uid], total_negative_samples
            )
    ...
    def get_encoder(self):
        try:
            return getattr(
                importlib.import_module("libcity.data.dataset.trajectory_encoder"),
                self.config["traj_encoder"],
            )(self.config)
