...
class CONVGCNDataset(TrafficStatePointDataset):
    def __init__(self, config):
        ...
        self.TG_in_one_day = self.config.get(
            "TG_in_one_day", 36
        )  # time interval of one day
        self.TG_in_one_week = self.config.get(
            "TG_in_one_week", 180
        ...
        self.parameters_str = (
            ...
            + "_"
            + str(self.TG_in_one_day)
            + "_"
            + str(self.TG_in_one_week)
        )
    ...
    def _generate_input_data(self, df):
        ...
        for index in range(
            self.TG_in_one_week, data.shape[0] - self.time_lag + 1 - self.output_window
        ):
            for i in range(self.num_nodes):
                temp1 = data[
                    index
                    - self.TG_in_one_week : index
                    + self.time_lag
                    - 1
                    - self.TG_in_one_week,
                    i,
        ...
        X = np.concatenate(
            (Xin[:, :, :, np.newaxis], Xout[:, :, :, np.newaxis]), axis=3
        )
        Yin = np.array(Yin)
        Yout = np.array(Yout)

        Y = np.concatenate(
            (Yin[:, :, :, np.newaxis], Yout[:, :, :, np.newaxis]), axis=3
        )
        X = X.swapaxes(1, 2)
        return X, Y
