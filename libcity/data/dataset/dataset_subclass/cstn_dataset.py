...
class CSTNDataset(TrafficStateGridOdDataset):
    ...
    def _generate_ext_data(self, ext_data):
        ...
        offsets = np.sort(
            np.concatenate(
                (np.arange(-self.input_window - self.output_window + 1, 1, 1),)
            )
        )
    ...
    def _generate_data(self):
        ...
        self._logger.info(
            "X shape: {}, W shape: {}, y shape: ".format(
                str(X.shape), str(W.shape), y.shape
            )
        )
        return X, W, y
    ...
    def _split_train_val_test(self, X, W, y):
        ...
        # eval
        x_eval, w_eval, y_eval = (
            X[num_train : num_train + num_eval],
            W[num_train : num_train + num_eval],
            y[num_train : num_train + num_eval],
        )
        # test
        ...
        self._logger.info(
            "train\tX: {}, W: {}, y: {}".format(
                str(x_train.shape), str(w_train.shape), str(y_train.shape)
            )
        )
        self._logger.info(
            "eval\tX: {}, W: {}, y: {}".format(
                str(x_eval.shape), str(w_eval.shape), str(y_eval.shape)
            )
        )
        self._logger.info(
            "test\tX: {}, W: {}, y: {}".format(
                str(x_test.shape), str(w_test.shape), str(y_test.shape)
            )
        )
    ...
    def _generate_train_val_test(self):
        ...
        (
            ...
            w_test,
            y_test,
        ) = self._split_train_val_test(X, W, y)
    ...
    def get_data(self):
        ...
        w_train[..., : self.output_dim] = self.scaler.transform(
            w_train[..., : self.output_dim]
        )
        y_train[..., : self.output_dim] = self.scaler.transform(
            y_train[..., : self.output_dim]
        )
        x_eval[..., : self.output_dim] = self.scaler.transform(
            x_eval[..., : self.output_dim]
        )
        w_eval[..., : self.output_dim] = self.scaler.transform(
            w_eval[..., : self.output_dim]
        )
        y_eval[..., : self.output_dim] = self.scaler.transform(
            y_eval[..., : self.output_dim]
        )
        x_test[..., : self.output_dim] = self.scaler.transform(
            x_test[..., : self.output_dim]
        )
        w_test[..., : self.output_dim] = self.scaler.transform(
            w_test[..., : self.output_dim]
        )
