...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        if os.path.exists(
            self.data_path + self.rel_file + ".rel"
        ):  # .rel file is not necessary
            self._load_rel()
        else:
            self.adj_mx = np.zeros(
                (len(self.geo_ids), len(self.geo_ids)), dtype=np.float32
            )
    ...
    def _load_grid_geo(self):
        ...
        for i in range(geofile.shape[0]):
            self.geo_to_rc[geofile["geo_id"][i]] = [
                geofile["row_id"][i],
                geofile["column_id"][i],
            ]
    ...
    def _load_rel(self):
        ...
        self._logger.info(
            "set_weight_link_or_dist: {}".format(self.set_weight_link_or_dist)
        )
        self._logger.info(
            "init_weight_inf_or_zero: {}".format(self.init_weight_inf_or_zero)
        )
        if self.weight_col != "":  # 根据weight_col确认权重列
            ...
            if (
                len(relfile.columns) > 5 or len(relfile.columns) < 4
            ):  # properties不只一列，且未指定weight_col，报错
                raise ValueError(
                    "Don't know which column to be loaded! Please set `weight_col` parameter!"
                )
        ...
        if (
            self.init_weight_inf_or_zero.lower() == "inf"
            and self.set_weight_link_or_dist.lower() != "link"
        ):
            self.adj_mx[:] = np.inf
        for row in self.distance_df.values:
            ...
            if self.set_weight_link_or_dist.lower() == "dist":  # 保留原始的距离数值
                ...
                if self.bidir_adj_mx:
                    self.adj_mx[self.geo_to_ind[row[1]], self.geo_to_ind[row[0]]] = row[
                        2
                    ]
        ...
        self._logger.info(
            "Loaded file " + self.rel_file + ".rel, shape=" + str(self.adj_mx.shape)
        )
        # 计算权重
        if self.distance_inverse and self.set_weight_link_or_dist.lower() != "link":
            self._distance_inverse()
        elif (
            self.calculate_weight_adj and self.set_weight_link_or_dist.lower() != "link"
        ):
        ...
        for i in range(self.len_row):
            for j in range(self.len_column):
                ...
                for d in dirs:
                    ...
                    if (
                        nei_i >= 0
                        and nei_i < self.len_row
                        and nei_j >= 0
                        and nei_j < self.len_column
                    ):
    ...
    def _load_dyna_3d(self, filename):
        ...
        # 求时间序列
        self.timesolts = list(
            dynafile["time"][: int(dynafile.shape[0] / len(self.geo_ids))]
        )
        ...
        data = np.array(
            data, dtype=np.float
        )  # (len(self.geo_ids), len_time, feature_dim)
        data = data.swapaxes(0, 1)  # (len_time, len(self.geo_ids), feature_dim)
        self._logger.info(
            "Loaded file " + filename + ".dyna" + ", shape=" + str(data.shape)
        )
        return data
    ...
    def _load_grid_3d(self, filename):
        ...
        # 求时间序列
        self.timesolts = list(
            gridfile["time"][: int(gridfile.shape[0] / len(self.geo_ids))]
        )
        self.idx_of_timesolts = dict()
        ...
        data = data.swapaxes(0, 1)  # (len_time, len(self.geo_ids), feature_dim)
        self._logger.info(
            "Loaded file " + filename + ".grid" + ", shape=" + str(data.shape)
        )
        return data
    ...
    def _load_grid_4d(self, filename):
        ...
        data = np.array(
            data, dtype=np.float
        )  # (len_row, len_column, len_time, feature_dim)
        data = data.swapaxes(2, 0).swapaxes(
            1, 2
    ...
    def _load_od_4d(self, filename):
        ...
        # 求时间序列
        self.timesolts = list(
            odfile["time"][: int(odfile.shape[0] / self.num_nodes / self.num_nodes)]
        )
        self.idx_of_timesolts = dict()
        ...
        data = data.transpose(
            (2, 0, 1, 3)
        )  # (len_time, num_nodes, num_nodes, feature_dim)
        self._logger.info(
            "Loaded file " + filename + ".od" + ", shape=" + str(data.shape)
        )
        return data
    ...
    def _load_grid_od_4d(self, filename):
        ...
        data = data.transpose(
            (2, 0, 1, 3)
        )  # (len_time, num_grids, num_grids, feature_dim)
        self._logger.info(
    ...
    def _load_ext(self):
        ...
        if not extfile["time"].isna().any():  # 时间没有空值
            self.ext_timesolts = list(
                map(lambda x: x.replace("T", " ").replace("Z", ""), self.ext_timesolts)
            )
        ...
        self._logger.info(
            "Loaded file " + self.ext_file + ".ext" + ", shape=" + str(df.shape)
        )
        return df
    ...
    def _add_external_information(self, df, ext_data=None):
        ...
        raise NotImplementedError(
            "Please implement the function `_add_external_information()`."
        )
    ...
    def _add_external_information_3d(self, df, ext_data=None):
        ...
        if self.add_time_in_day and not is_time_nan:
            time_ind = (
                self.timesolts - self.timesolts.astype("datetime64[D]")
            ) / np.timedelta64(1, "D")
        ...
        if ext_data is not None:
            if not is_time_nan:
                ...
                if ext_data.shape[0] == df.shape[0]:
                    ...
                    for i in range(select_data.shape[1]):
                        data_ind = select_data[:, i]
                        data_ind = np.tile(data_ind, [1, num_nodes, 1]).transpose(
                            (2, 1, 0)
                        )
                        data_list.append(data_ind)
    ...
    def _add_external_information_4d(self, df, ext_data=None):
        ...
        if self.add_time_in_day and not is_time_nan:
            ...
            time_in_day = np.tile(time_ind, [1, len_row, len_column, 1]).transpose(
                (3, 1, 2, 0)
            )
            data_list.append(time_in_day)
        if self.add_day_in_week and not is_time_nan:
        ...
        if ext_data is not None:
            if not is_time_nan:
                ...
                for i in range(select_data.shape[1]):
                    data_ind = select_data[:, i]
                    data_ind = np.tile(data_ind, [1, len_row, len_column, 1]).transpose(
                        (3, 1, 2, 0)
                    )
                    data_list.append(data_ind)
            else:  # 没有给出具体的时间戳，只有外部数据跟原数据等长才能默认对接到一起
                if ext_data.shape[0] == df.shape[0]:
                    ...
                    for i in range(select_data.shape[1]):
                        data_ind = select_data[:, i]
                        data_ind = np.tile(
                            data_ind, [1, len_row, len_column, 1]
                        ).transpose((3, 1, 2, 0))
                        data_list.append(data_ind)
        data = np.concatenate(data_list, axis=-1)
        return data
    ...
    def _add_external_information_6d(self, df, ext_data=None):
        ...
        if self.add_time_in_day and not is_time_nan:
            ...
            time_in_day = np.tile(
                time_ind, [1, len_row, len_column, len_row, len_column, 1]
        ...
        if self.add_day_in_week and not is_time_nan:
            ...
            day_in_week = np.zeros(
                shape=(num_samples, len_row, len_column, len_row, len_column, 7)
            )
            day_in_week[np.arange(num_samples), :, :, :, :, dayofweek] = 1
            data_list.append(day_in_week)
        # 外部数据集
        if ext_data is not None:
            if not is_time_nan:
                ...
                for i in range(select_data.shape[1]):
                    data_ind = select_data[:, i]
                    data_ind = np.tile(
                        data_ind, [1, len_row, len_column, len_row, len_column, 1]
                    ).transpose((5, 1, 2, 3, 4, 0))
                    data_list.append(data_ind)
            else:  # 没有给出具体的时间戳，只有外部数据跟原数据等长才能默认对接到一起
                if ext_data.shape[0] == df.shape[0]:
    ...
    def get_data(self):
        ...
        if self.normal_external:
            x_train[..., self.output_dim :] = self.ext_scaler.transform(
                x_train[..., self.output_dim :]
            )
            y_train[..., self.output_dim :] = self.ext_scaler.transform(
                y_train[..., self.output_dim :]
            )
            x_val[..., self.output_dim :] = self.ext_scaler.transform(
                x_val[..., self.output_dim :]
            )
            y_val[..., self.output_dim :] = self.ext_scaler.transform(
                y_val[..., self.output_dim :]
            )
            x_test[..., self.output_dim :] = self.ext_scaler.transform(
                x_test[..., self.output_dim :]
            )
            y_test[..., self.output_dim :] = self.ext_scaler.transform(
                y_test[..., self.output_dim :]
            )
