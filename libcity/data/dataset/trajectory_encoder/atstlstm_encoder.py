...
class AtstlstmEncoder(AbstractTrajectoryEncoder):
    ...
    def __init__(self, config):
        ...
        self.geo = pd.read_csv(
            os.path.join(self.data_path, "{}.geo".format(self.geo_file))
        )
        ...
        for i, traj in enumerate(trajectories):
            ...
            for index, row in enumerate(traj):
                ...
                lon, lat = parse_coordinate(
                    self.geo.loc[self.geo["geo_id"] == loc].iloc[0]["coordinates"]
                )
                if index == 0:
                    ...
                    tim_interval.append(
                        100
                    )  # choose the same fixed value as the reference code
                    ...
                    tim_interval.append(
                        datetime.timestamp(now_time) - datetime.timestamp(pre_time)
                    )
                    loc_distance.append(
                        distance.distance((pre_lat, pre_lon), (lat, lon)).kilometers
                    )
                pre_time = now_time
                pre_lat = lat
                pre_lon = lon
            ...
            row = traj[-2]
            loc = row[4]
            pre_lon, pre_lat = parse_coordinate(
                self.geo.loc[self.geo["geo_id"] == loc].iloc[0]["coordinates"]
            )
            for neg in negative_sample[i]:
                neg_lon, neg_lat = parse_coordinate(
                    self.geo.loc[self.geo["geo_id"] == neg].iloc[0]["coordinates"]
                )
                ...
                neg_time.append(
                    tim_interval[-1]
                )  # use target's time interval as the neg sample's
                neg_distance.append(
                    distance.distance((neg_lat, neg_lon), (pre_lat, pre_lon)).kilometers
                )
            trace = []
    ...
    def gen_data_feature(self):
        loc_pad = self.loc_id
        self.pad_item = {"current_loc": loc_pad, "current_dis": 0.0, "current_tim": 0.0}
        self.data_feature = {
