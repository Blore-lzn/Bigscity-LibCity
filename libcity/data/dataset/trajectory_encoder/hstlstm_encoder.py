...
class HstlstmEncoder(AbstractTrajectoryEncoder):
        ...
        for index, traj in enumerate(trajectories):
            ...
            for index, point in enumerate(traj):
                ...
                if index == 0:
                    ...
                    tim_interval.append(
                        self.tim_interval_init
                    )  # use fixed interval for start point
                    ...
                    if loc not in self.location2id:
                        ...
                        self.loc_id += 1
                    current_loc.append(self.location2id[loc])
                    tim_diff = datetime.timestamp(now_time) - datetime.timestamp(
                        pre_time
                    )
                    if tim_diff > self.tim_max:
                        self.tim_max = tim_diff
                    tim_interval.append(tim_diff)
                    dis_diff = distance.distance(
    ...
    def gen_data_feature(self):
        loc_pad = self.loc_id
        self.pad_item = {"current_loc": loc_pad, "tim_interval": 0.0, "dis": 0.0}
        self.data_feature = {
