...
class TtpnetEncoder(AbstractETAEncoder):
    def __init__(self, config):
        ...
        self.feature_dict = {
            ...
            "current_loc": "int",
            "current_tim": "float",
            "masked_current_tim": "float",
            "current_dis": "float",
            "speeds": "float",
            "speeds_relevant1": "float",
            "speeds_relevant2": "float",
            "speeds_long": "float",
            "grid_len": "float",
            "uid": "int",
            "weekid": "int",
    ...
    def encode(self, uid, trajectories, dyna_feature_column):
        ...
        for traj in trajectories:
            ...
            begin_time = datetime.strptime(
                traj[0][dyna_feature_column["time"]], "%Y-%m-%dT%H:%M:%SZ"
            )
            end_time = datetime.strptime(
                traj[-1][dyna_feature_column["time"]], "%Y-%m-%dT%H:%M:%SZ"
            )
            weekid = int(begin_time.weekday())
            timeid = (
                int(begin_time.strftime("%H")) * 4
                + int(begin_time.strftime("%M")) // 15
            )
            ...
            for point in traj:
                ...
                if "current_dis" in dyna_feature_column:
                    ...
                    dis = (
                        geo_distance(current_longi[-2], current_lati[-2], longi, lati)
                        + last_dis
                    )
                    last_dis = dis
                current_dis.append(dis)

                tim = datetime.strptime(
                    point[dyna_feature_column["time"]], "%Y-%m-%dT%H:%M:%SZ"
                )
            ...
            if "current_dis" in dyna_feature_column:
                dist = (
                    traj[-1][dyna_feature_column["current_dis"]]
                    - traj[0][dyna_feature_column["current_dis"]]
                )
            else:
                dist = last_dis
    ...
    def gen_scalar_data_feature(self, train_data):
        ...
        for data in train_data:
            ...
            dist_gap = list(
                map(lambda x: x[0] - x[1], zip(dist_gap[1:], dist_gap[:-1]))
            )
            dist_gap_list.extend(dist_gap)
            time_gap = data[scalar_feature_column["current_tim"]][:traj_len]
            time_gap = list(
                map(lambda x: x[0] - x[1], zip(time_gap[1:], time_gap[:-1]))
            )
            time_gap_list.extend(time_gap)
            speeds_list.extend(data[scalar_feature_column["speeds"]])
            speeds_relevant1_list.extend(
                data[scalar_feature_column["speeds_relevant1"]]
            )
            speeds_relevant2_list.extend(
                data[scalar_feature_column["speeds_relevant2"]]
            )
