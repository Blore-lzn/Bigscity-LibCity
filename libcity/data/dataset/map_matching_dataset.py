...
class MapMatchingDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.delta_time = config.get(
            "delta_time", True
        )  # True则轨迹输入时间差(s)，False则轨迹输入时间datetime.datetime
        self.with_rd_speed = "speed" in config["rel"]["geo"].keys()

        # cache
        self.cache_dataset = self.config.get("cache_dataset", True)
        self.parameters_str = str(self.dataset) + "_" + str(self.delta_time)
    ...
    def _load_geo_and_rel(self):
        ...
        if not ["LineString"] == self.config["geo"]["including_types"]:
            raise ValueError(
                ".geo file should include LineString type in Map Matching task!"
            )
        ...
        for index, row in geofile.iterrows():
            ...
            if origin_node not in self.rd_nwk.nodes:
                self.rd_nwk.add_node(
                    origin_node, lon=coordinate[0][0], lat=coordinate[0][1]
                )
            if dest_node not in self.rd_nwk.nodes:
                self.rd_nwk.add_node(
                    dest_node, lon=coordinate[1][0], lat=coordinate[1][1]
                )
        ...
        # logger
        self._logger.info(
            "Loaded file " + self.geo_file + ".geo" + ", num_nodes=" + str(geo_num)
        )
        self._logger.info(
            "Loaded file " + self.rel_file + ".rel, num_roads=" + str(len(self.rd_nwk))
        )
    ...
    def _load_usr(self):
        ...
        self._logger.info(
            "Loaded file " + self.rel_file + ".usr, num_users=" + str(len(self.usr_lst))
        )
    ...
    def _load_dyna(self):
        ...
        if not ["trajectory"] == self.config["dyna"]["including_types"]:
            raise ValueError(
                ".dyna file should include trajectory type in Map Matching task!"
            )
        ...
        for index, row in dynafile.iterrows():
            ...
            if usr_id not in self.usr_lst:
                raise ValueError(
                    "entity_id %d should be in usr_ids in Map Matching task!" % usr_id
                )
            ...
            if self.with_time:
                if usr_id in self.trajectory.keys():
                    if traj_id in self.trajectory[usr_id].keys():
                        self.trajectory[usr_id][traj_id].append(
                            [dyna_id] + coordinate + [parse_time(time)]
                        )
                    else:
                        self.trajectory[usr_id][traj_id] = [
                            [dyna_id] + coordinate + [parse_time(time)]
                        ]
                else:
                    self.trajectory[usr_id] = {
                        traj_id: [[dyna_id] + coordinate + [parse_time(row["time"])]]
                    }
        ...
        self._logger.info(
            "Loaded file "
            + self.dyna_file
            + ".dyna, num of GPS samples="
            + str(dynafile.shape[0])
        )
    ...
    def _load_truth_dyna(self):
        ...
        for index, row in truth_dyna.iterrows():
            ...
            if usr_id not in self.usr_lst:
                raise ValueError(
                    "usr_id %d should be in usr_ids in Map Matching task!" % usr_id
                )
