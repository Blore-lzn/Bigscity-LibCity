...
class ETADataset(AbstractDataset):
    ...
    def _get_encoder(self):
        try:
            return getattr(
                importlib.import_module("libcity.data.dataset.eta_encoder"),
                self.config["eta_encoder"],
            )(self.config)
    ...
    def _load_dyna(self):
        ...
        dyna_file = pd.read_csv(
            os.path.join(self.data_path, "{}.dyna".format(self.dyna_file))
        )
        self._logger.info(
            "Loaded file " + self.dyna_file + ".dyna, shape=" + str(dyna_file.shape)
        )
        ...
        if self.need_cut:
                ...
                for uid in tqdm(user_set, desc="cut and filter trajectory"):
                    ...
                    for index, row in usr_traj.iterrows():
                        ...
                        if index == 0:
                            ...
                            if (
                                time_off < window_size
                                and time_off >= 0
                                and len(session) < max_session_len
                            ):
                                session.append(row.tolist())
                            else:
            ...
            for dyna in dyna_file.itertuples():
                ...
                if entity_id not in res:
                    res[entity_id] = []
                rows = dyna_file[
                    (dyna_file["entity_id"] == entity_id)
                    & (dyna_file["traj_id"] == traj_id)
                ]
    ...
    def _encode_traj(self, data):
        ...
        for uid in tqdm(data, desc="encoding trajectory"):
            encoded_data[str(uid)] = self.encoder.encode(
                int(uid), data[uid], self.dyna_feature_column
            )
    ...
    def _divide_data(self):
        ...
        for uid in tqdm(user_set, desc="dividing data"):
            ...
            eval_num = math.ceil(traj_len * (train_rate + eval_rate))
        ...
        if self.data is None:
            if self.config["cache_dataset"] and os.path.exists(
                ...
                if self.need_cut and os.path.exists(self.cut_data_cache):
                    ...
                    self.dyna_feature_column = {
                        col: i for i, col in enumerate(dyna_file)
                    }
