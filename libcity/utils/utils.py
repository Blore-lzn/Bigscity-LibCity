...
def get_executor(config, model, data_feature):
    ...
    """
    try:
        return getattr(importlib.import_module("libcity.executor"), config["executor"])(
            config, model, data_feature
        )
...
def get_model(config, data_feature):
    ...
    if config["task"] == "traj_loc_pred":
        try:
            return getattr(
                importlib.import_module("libcity.model.trajectory_loc_prediction"),
                config["model"],
            )(config, data_feature)
        except AttributeError:
        ...
        try:
            return getattr(
                importlib.import_module("libcity.model.traffic_flow_prediction"),
                config["model"],
            )(config, data_feature)
        except AttributeError:
            try:
                return getattr(
                    importlib.import_module("libcity.model.traffic_speed_prediction"),
                    config["model"],
                )(config, data_feature)
            except AttributeError:
                try:
        ...
        try:
            return getattr(
                importlib.import_module("libcity.model.map_matching"), config["model"]
            )(config, data_feature)
        except AttributeError:
            raise AttributeError("model is not found")
        ...
        try:
            return getattr(
                importlib.import_module("libcity.model.road_representation"),
                config["model"],
            )(config, data_feature)
        except AttributeError:
            raise AttributeError("model is not found")
        ...
        try:
            return getattr(
                importlib.import_module("libcity.model.eta"), config["model"]
            )(config, data_feature)
        except AttributeError:
            raise AttributeError("model is not found")
    else:
...
def get_evaluator(config):
    ...
    try:
        return getattr(
            importlib.import_module("libcity.evaluator"), config["evaluator"]
        )(config)
    except AttributeError:
...
def get_logger(config, name=None):
    ...
    log_filename = "{}-{}-{}-{}.log".format(
        config["exp_id"], config["model"], config["dataset"], get_local_time()
    )
