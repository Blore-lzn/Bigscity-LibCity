...
class VisHelper:
    def __init__(self, _config):
        ...
        if (
            "dyna" in self.data_config
            and ["state"] == self.data_config["dyna"]["including_types"]
        ):
            self.type = "state"
        elif (
            "grid" in self.data_config
            and ["state"] == self.data_config["grid"]["including_types"]
        ):
            self.type = "grid"
        elif (
            "dyna" in self.data_config
            and ["trajectory"] == self.data_config["dyna"]["including_types"]
        ):
    ...
    def _visualize_state(self):
        ...
        dyna_feature_lst = [
            _ for _ in list(dyna_file.columns) if _ not in self.dyna_reserved_lst
        ]
        ...
        json.dump(
            geojson_obj,
            open(self.save_path + "/" + save_name, "w", encoding="utf-8"),
            ensure_ascii=False,
            indent=4,
        )
    ...
    def _visualize_grid(self):
        ...
        grid_feature_lst = [
            _ for _ in list(grid_file.columns) if _ not in self.grid_reserved_lst
        ]
        ...
        for _, row in geo_file.iterrows():
            ...
            feature_dct = row[geo_feature_lst].to_dict()
            dyna_i = grid_file[
                (grid_file["row_id"] == row_id) & (grid_file["column_id"] == column_id)
            ]
